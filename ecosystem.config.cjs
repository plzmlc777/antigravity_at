...
const agentApps = [
    ...
    },
    {
        ...
        // 은 하루 1회 사이클에서만 실행돼 트리거 후 최대 24시간 방치됐다.
        // 드라이버는 정본 없이도 HARD_EXIT_DAYS=31 을 스스로 판정하므로
        // 매시간 돌리면 그 지연이 1시간이 된다.
        //
        // ⚠ 전략을 바꾸지 않는다 — "정본이 이미 내린 결론을 더 빨리 집행"할
        //    뿐이다. 정본 평가 주기는 그대로 하루 1회다. 상태가 안 바뀌면
        //    매 실행이 `in sync` 무동작이다.
        // ⚠ 관문을 먼저 통과해야 주문한다 (일봉 사이클과 같은 순서).
        // ⚠ 가격 기반 청산(손절·익절)은 거래소 브래킷이 틱 단위로 감시한다 —
        //    이 잡과 무관하다.
        //
        // 일봉 사이클(02:30 UTC)과 겹치지 않게 매시 :20 에 돈다.
        name: "lifecycle-hourly-reconcile",
        script: SAS_WRAPPER,
        args: `'20 * * * *' ./backend/scripts/binance/run_lifecycle_hourly_reconcile.sh`,
        interpreter: "bash",
        cwd: ".",
        autorestart: true,
        max_restarts: 10,

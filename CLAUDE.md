...
## Ubuntu Remote Server Deployment (로컬 네트워크)

> 24시간 운영을 위한 로컬 Ubuntu 서버 배포
...
| User | admin-ubuntu |
| Path | ~/ai/antigravity/auto_trading |
| SSH | `ssh admin-ubuntu@172.30.1.77` |

### 1. DB 백업 및 전송

**로컬(WSL)에서 DB 백업:**
```bash
wsl -e bash -c 'PGPASSWORD=antigravity_password pg_dump -U antigravity_user -h localhost antigravity_db > ~/antigravity_db_full_backup.sql'
```

**Ubuntu 서버로 전송:**
```bash
wsl -e bash -c 'scp ~/antigravity_db_full_backup.sql admin-ubuntu@172.30.1.77:~/'
```

### 2. DB 복원 (전체 동기화 시)

> ⚠️ 기존 데이터가 덮어씌워집니다. 필요시 기존 DB 먼저 백업!

```bash
# 기존 DB 백업
ssh admin-ubuntu@172.30.1.77 'PGPASSWORD=antigravity_password pg_dump -U antigravity_user -h localhost antigravity_db > ~/antigravity_db_old_backup_$(date +%Y%m%d_%H%M%S).sql'

# 스키마 리셋 및 복원
ssh admin-ubuntu@172.30.1.77 'PGPASSWORD=antigravity_password psql -U antigravity_user -h localhost -d antigravity_db -c "DROP SCHEMA public CASCADE; CREATE SCHEMA public; GRANT ALL ON SCHEMA public TO antigravity_user;"'

ssh admin-ubuntu@172.30.1.77 'PGPASSWORD=antigravity_password psql -U antigravity_user -h localhost -d antigravity_db < ~/antigravity_db_full_backup.sql'
```
...
**Quick Deploy:**
```bash
ssh admin-ubuntu@172.30.1.77 "cd ~/ai/antigravity/auto_trading && git fetch origin && git reset --hard origin/master"
```

### 4. 의존성 설치 & PM2 재시작

```bash
ssh admin-ubuntu@172.30.1.77 "cd ~/ai/antigravity/auto_trading && ./deploy_with_pm2.sh"
```
...
```bash
# 버전 확인
ssh admin-ubuntu@172.30.1.77 'curl -s http://localhost:8001/api/v1/system/version'

# PM2 상태 확인
ssh admin-ubuntu@172.30.1.77 'cd ~/ai/antigravity/auto_trading && npm run status'

# 로그 확인
ssh admin-ubuntu@172.30.1.77 'cat ~/.pm2/logs/at-backend-out.log | tail -20'
```

### Access Points

- **Frontend**: http://172.30.1.77:5173
- **Backend API**: http://172.30.1.77:8001
- **API Docs**: http://172.30.1.77:8001/docs

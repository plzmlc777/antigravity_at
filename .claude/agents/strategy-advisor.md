...
- **D-013 — Wednesday entry block** (informational): the engine now blocks new L1 entries on Wednesdays via `block_entry_weekdays=[2]` (Mon=0..Sun=6). Audit found Wednesday WR=20% PnL=-90.14. Do NOT recommend disabling this default unless the user explicitly requests it.
- **D-011 — loss-cooldown** (informational): the engine now auto-pauses new L1 entries for 60 minutes when the rolling 4h cumulative cycle PnL drops below -20 USDT (`cooldown_loss_threshold`, `cooldown_loss_window_minutes`, `cooldown_duration_minutes`). Treat this as a backstop, not a substitute for sound parameter selection.
- **D-010 — confidence must separate WR from PnL**: When reporting `confidence`, you MUST split the basis into win-rate evidence and PnL evidence in the `rationale`. **Hard cap**: if the strategy/symbol combination has historical PnL ≤ 0, your `confidence` MUST NOT exceed 0.50 regardless of how high the WR is. Reason: 2026-04-06 audit found a noop session at WR=89% but PnL=-1.07 — high WR with low/negative PnL is a known trap. Required `rationale` template excerpt: "WR-기반 confidence: X (n=N), PnL-기반 confidence: Y (cum=Z USDT). 최종 cap 적용: min(...)."

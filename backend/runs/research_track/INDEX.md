...
**마지막 갱신**: 2026-05-20 (**paradigm 110 alt_cohort_dispersion_compression_percentile_rank_directional_4h** BROAD_FALSIFIED_FEE_FLOOR (mechanism direction INVERTED) — /new-paradigm-frontier continuous-parallel 8th dispatch, paradigm 109 rescue R1 percentile_rank reformulation. **Structural fix SUCCESS**: p_rank ≤ 0.10 guarantees 11.51% trigger rate (paradigm 109 z ≤ −2 0% 해소). **Mechanism direction INVERTED 결정적 입증**: A_focus (compression + BTC up → cohort LONG) gross **−12.94bp** / t=**−6.40** / **13/13 alts ci_neg** / perm_p 0.000. A_mirror (compression + BTC up → cohort SHORT) gross +12.94bp 9/10 quarters pos_t + 4/13 syms ci_pos = **mechanism REAL but fee-bound** (16bp 압축). B_same_pr0.15_h1440 단일 sweep PASS (gross +25.35bp net +17.35bp sigex 2.43 perm_p 0.074 ci+0.63bp) BUT life-changing edge **0.17%/trade ≪ 2%** (11.8x 미달) NARROW_SCOPE 자격 불가. **Lesson #40 candidate CONFIRMED 2nd dogfood** — structural threshold feasibility (109 halt) vs mechanism viability (110 inverted) 독립 차원 입증, R-0 sequential prescreen 의무. **Lesson #39 candidate CONFIRMED 자격 2nd dogfood** (108 perfect symmetry broad-neg + 110 perfect symmetry mechanism-inverted = sub-class refinement). **NEW Lesson #41 candidate** (1st dogfood) — compression-regime-conditional-on-BTC-direction-inverts (cohort = mean reversion or alt-vs-BTC rotation in compression regime, not BTC follow). **Lesson #32 negative-drift artifact 새 sub-pattern** (A_focus −20.94 < B baseline −16.42, paradigm 101 positive-drift와 dual). No formal family retire (mechanism real 109+110 different fail modes). 110번째 graveyard. R-1 wall-clock 38s.)

**이전 갱신**: 2026-05-20 (**paradigm 109 alt_cohort_dispersion_compression_directional_4h** SAMPLE_INSUFFICIENT_STRUCTURAL_THRESHOLD_INFEASIBLE — /new-paradigm-frontier continuous-parallel 7th dispatch. Cross-section return dispersion compression z_disp ≤ −2 가설 R-0 prescreen halt (R-1 미실행, wall-clock 4분). **Structural finding**: σ_cs 비음수 by construction → rolling-30d z-score 분포 lower tail structurally bounded. 실측 z_disp.min() = **−1.92** across 2.36yr panel × 14 syms (joblib cache PASS, substrate audit PASS 100% data window). z ≤ −2.0 trigger **0 events** (z ≤ −1.5 only 7, z ≤ −1.0 = 1,323). Lesson #11/#23 downstream FAIL but **root cause structural threshold infeasibility** distinct fail mode. **NEW Lesson #40 candidate `non_negative_aggregate_zscore_one_sided_floor`** 1st dogfood — non-negative aggregate statistics (std/var/count/magnitude/dispersion/ATR/|return|/drawdown) symmetric z ≤ −T threshold structurally impossible, percentile rank / log-transform / ratio compression / absolute threshold으로 reformulate 필요. Lesson #28 substrate audit PASS + #30 100% + #34 1st use as structural-threshold-feasibility prescreen tool (not just trigger rate). Orchestrator R-0 design defect dogfood: Lesson #34 trigger rate only 적용 → threshold feasibility 차원 누락. Rescue path R1 percentile_rank_compression (σ_cs ≤ rolling_30d_p10 guaranteed 10% trigger) candidate paradigm 110 가능 (statistic novelty preserved). 109번째 graveyard. R-1 미실행.)

**이전 갱신**: 2026-05-20 (**paradigm 108 intra_symbol_spot_perp_lead_lag_alt_5m** BROAD_FALSIFIED_FEE_FLOOR — /new-paradigm-frontier continuous-parallel 6th dispatch (103+104+105+106+107+108). Intra-exchange (Binance) cross-venue (spot vs perp) 5m frame cross-correlation lag function — 14 syms × 1.99yr (data_window_ratio 83%) × premium_index joblib 분해 (spot = perp/(1+premium)). **5-axis NOVEL 3/5 ex ante** (data source intra-sym leg decomposition + statistic xcorr lag function + universe intra-sym bilateral). R-0 prescreen Lesson #34 CRITICAL FINDING: **mean|τ*|=0 for 90% windows / mean|corr@τ=0|=0.981** on BTC 6m — Binance internal market-maker arbitrage closes spot↔perp basis within sub-bar timescale. R-1 4-quadrant SNT 모두 FAIL (A_focus gross +2.08bp net −13.92bp sigex −1.107 ci_lower −15.94bp perm_p 0.177 / A_mirror exact −2.08bp **perfect symmetry**). 45-cell hold×threshold sweep **0/45 PASS** (best gross +5.51bp ≪ 16bp fee floor 2.9x). Concentration **0/9 quarters t>0** (all t∈[−6.20,−2.66]) + **0/14 syms ci_pos** broad uniform negative. **Lesson #21 axis-alone dogfood explicit SUCCESS**: corr_alone 4.03bp > joint 2.94bp + tau_alone 3.04bp > joint — joint signal WEAKER than single axes, definitive no-synthesis proof. Life-changing 4-dim 1/4 PASS (trades/yr 2588 ✅ / util 29.5% ❌ / edge **−0.139%** ❌ / sharpe **−9.3** ❌). **NEW Lesson #38 candidate**: Same-venue arbitrage tightness antipattern — intra-exchange spot↔perp 5m frame lead-lag structurally fee-floor bound (mean|corr@τ=0| > 0.98 = arbitrage too tight, sub-bar closure). **NEW Lesson #39 candidate**: Symmetric perfect mirror antipattern — A_focus ±2.08bp exact = trigger zero directional info, pure direction-bet trap (distinct from Lesson #19 SNT 8bp asymmetry fail mode). Family-distinct from 8th cross-exchange single-axis retire (intra-exchange vs cross-exchange) verified. New advisory caution (1-dispatch only, formal retire 보류) `intra_exchange_cross_venue_5m_frame_lead_lag` — 2nd dispatch needed for Tier 4 formal. Adjacent variants still open (1m frame a fortiori tighter; cross-exchange variant substrate prep; different statistic mark-index leg decomposition). 108번째 graveyard. R-1 wall-clock 48.4s + R-0 ~30s = 80s total.)

**이전 갱신**: 2026-05-20 (**paradigm 107 dart_form5_extreme_stake_disposal_kr_equity_continuation_short_d5** NARROW_SCOPE_LIFE_CHANGING_FAIL (verdict revised from auto BROAD_FALSIFIED via off-primary cell discovery) — paradigm 106 inverted finding follow-up. **Off-primary cell thr=3.0%×hold=3d A_focus_sell_short 3-gate PASS**: gross +193.30bp / net +158.30bp / sigex **+3.51** / ci_lower **+7.7bp** / perm_p 0.0000 + cross-proxy gap_neg×SHORT sigex +4.43 ci+168bp. 그러나 Lesson #20 4-cond 2/4 PASS only (Concentration FAIL only 2 sym measurable / hold sweep non-monotonic D+5→D+10 sign-flip) + life-changing 4-dim 3/4 PASS (edge **1.583%<2.0%** 0.42pp 미달, trades/yr 56 util 0.32 sharpe 1.60 PASS). **Lesson #36 CONFIRMED PARTIAL** — magnitude-direction inversion REAL for SELL-side at narrow cell (D+3 sweet spot, mean-reverts by D+10+). paradigm 106 inverted directionally correct but wrong hold horizon. **9th family retire DEFERRED** — KR equity disclosure family 6 sub-class 5 axes test 후 not 5-axis-exhausted decisive (real mechanism but blocked by sample density 106 events/67 stocks + edge gap). **NEW Lesson candidate**: Verdict logic must scan full hold×threshold sweep for 3-gate PASS cells (off-primary discovery), not only primary — auto-evaluator missed paradigm 107 off-primary PASS. Lesson #35 NARROW_SAMPLE_LIMITED 새 sub-class. NARROW_SCOPE_LIFE_CHANGING_FAIL 4th dogfood (95+99+104+107). 107번째 graveyard.)

**이전 갱신**: 2026-05-19 (**paradigm 106 kind_block_trade_announce_kr_equity_mean_reversion_long_d1** BROAD_FALSIFIED_FEE_FLOOR — KIND public scraping 차단 → DART Form 5 (주식등의대량보유상황보고서) fallback (Lesson #28 amendment 6th dogfood substrate-rerouting SUCCESS). Track 3 350 universe × 2,075 events × 9 quarters all-measurable (paradigm 100 Q1-cluster trap 정확 회피). Primary thr=1.0% × hold=1d × A_focus_sell_long: gross −16.16bp / net −51.16bp / sig_t_excess −0.49 / perm_p 0.697 FAIL 3-gate. **Mean-reversion 가설 structurally inverted at extreme thresholds**: thr=3.0% × hold=5d A_mirror_sell_short gross +166.7bp net +131.7bp sig_t_excess +2.92 (continuation direction at concentrated institutional sells) BUT 3-gate FAIL ci_lower<0. Lesson #32 universe-baseline-coherent trap **1st positive dogfood SUCCESS** (A_focus -51.2bp < B_baseline -34.1bp = -17.0bp drift). Lesson #35 fee-trap classification **2nd dogfood** (paradigm 104+106) → promote candidate → confirmed 자격. Concentration Gate both-quadrant FAIL (1/16 sym ci_pos / 1/9 quarter pos_t). **KR equity disclosure family 6 sub-class 6 graveyards 결정적 강화 path exhausted**: paradigm 92+93+100+101+102+106 모든 axes (directional momentum + supply contract attention + supply vol expansion + Form 5 mean-reversion) graveyard. Mechanism redefinition note: Form 5 (5% holding change) ≠ KIND after-hours block trade — substrate proxy substitution 차후 paradigm spec 명시 의무. 106번째 graveyard.)

**이전 갱신**: 2026-05-19 (**paradigm 105 cross_exchange_funding_spread_binance_bitget_alt_directional_8h_illiquid_venue** DISPATCH_IMPOSSIBLE — paradigm 103 follow-up path #1 (illiquid venue funding spread). **Substrate-time-dimension absent**: Bitget V2 33d/100row hard-cap (모든 param 변형 무효), Bitget V1 decommissioned HTTP 30032, Gate.io V4 HTTP 401 auth required (freemium blocked), OKX V5 ~90d cursor walk-back only. data_window_ratio 33d/912d=3.6% ≪ 30% Lesson #30 advisory non-actionable. R-1 미실행. **NEW Lesson #28 amendment**: substrate-time-dimension (historical depth) vs substrate-existence-dimension (event-time presence, paradigm 89) 양면 정식 분리 — 5th dogfood. **Cross-exchange family Tier 4 formal retire 정당화**: 3/3 attempts fail (103 fee-trap top-tier×top-tier / 104 OI top-tier×top-tier pool-drift+life-changing FAIL / 105 substrate-time-dimension mid-tier), 잔여 axis paid feed (Kaiko/Amberdata/Coinglass) freemium policy 영구 차단. Lesson #11 8th dogfood + Lesson #30 3rd dogfood + Lesson #34 prescreen 미적용 (substrate halt). **Lesson #35 candidate** confirmation 보류 (R-1 미실행). 105번째 graveyard, wall-clock <5분 (substrate audit only).)

**이전 갱신**: 2026-05-19 (**paradigm 104 cross_exchange_oi_level_differential_binance_bybit_alt_directional_4h** BROAD_FALSIFIED primary 240m + NARROW_SCOPE_LIFE_CHANGING_FAIL at extended hold — paradigm 103 follow-up path #3 (OI level differential, rates 아닌). **Path #3 premise 구조적 입증**: gross 25.7bp (A_focus z=2.5 240m) = paradigm 103 14bp ceiling 대비 1.84x 강함, 1440m gross 92.78bp = 3.87x 강함. **Primary 240m fail mode = upward-bias pool drift** (perm_p 0.988 / fee-floor 아님, paradigm 103과 다른 trap). 480m + 1440m three-gate + Concentration PASS (4/7 sym ci_pos, 8/10 quarter pos_t) but **life-changing edge 0.26-0.77%/trade FAIL ≥2.0%** 4-dim. **Lesson #34 confirmed (2nd dogfood)** empirical z 분포 prescreen 효과 paradigm 103+104 모두 검증 → promote candidate→confirmed. **NEW Lesson #35 candidate**: Primary-hold-fee-trap vs upward-bias-pool-drift dual fail mode distinction (eval_research_gate triage 분리 의무). Cross-exchange family path #2 (rates) + path #3 (OI level) 누적 2개 graveyards → cross-exchange family advisory caution 등급 상향 권고 (path #1 illiquid venue 미검증). **신규 인프라 영구 자산**: Binance + Bybit OI joblib cache 1h 7 syms × 2yr (~14MB). 104번째 graveyard. R-1 wall-clock ~5분 (backfill 325s + R-1 ~30s).)

**이전 갱신**: 2026-05-19 (**paradigm 103 cross_exchange_funding_spread_binance_bybit_alt_directional_8h** BROAD_FALSIFIED_FEE_FLOOR — first dispatch under continuous-parallel policy. Bybit V5 substrate verified 7/7 deep-syms × 2.5yr (joblib cache 영구 자산, future cross-exchange paradigm 재사용 가능). Initial 50bp 가정 vs 실측 p99=3bp 1-2 orders 어긋남 → recalibrated bp=1.0. 4-quadrant SNT: A_focus gross +11.83bp/sigex +1.94 / B_focus +14.33bp/sigex +2.87 — focus direction GROSS-positive (메커니즘 정성적 정확) BUT 16bp fee floor 압축 모두 net < 0. Concentration Gate 전 cell FAIL 0/7 ci_pos. Hold sweep mechanism-revealing 비대칭: A_focus 240→1440m 단조 개선 (−4 → +24bp long-side slow continuation drift) vs B_focus 단조 악화. **NEW lesson #34 candidate**: cross-exchange single-statistic spread on liquid USDT perps is fee-floor bound by venue arbitrage efficiency — Binance↔Bybit liquid perp pair는 8h cycle spread differential을 venue arb가 fee-tolerance까지 평활화. **Funding family Tier 4 retire 강화 입증** — memory snapshot §3 cross-exchange axis exception 1회 invoke + falsify. 향후 family-distinct path 후보 3개 (less liquid venue pair / lead-lag delay study / cross-exchange OI divergence). 103번째 graveyard.)

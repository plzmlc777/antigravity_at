...
| (open) | 새 mechanism candidate 발의 필요 | — | — | Q3 §6.2 lessons #8-#21 회피. **Cross-asset corr (74-77) + geometric path metrics (78) + funding × OI joint squeeze (73, 79) family 폐기 확정**. **5m microstructure single-domain (paradigm 80 level joint + paradigm 82 velocity × direction event + paradigm 83 latent k-means per-sym) 3개 연속 broad-falsified로 advisory caution 격상** (Tier 4 formal retire 직전, 단 새 transform class — cross-exchange dispersion / 시간 lagged cross-feature / aggTrades event detection — 가능성은 살아있음). cross-symbol rolling beta breakdown은 narrow scope (3 syms cluster) 외 falsified (81, lesson #20). 신규 feature transform (premium acceleration / OI dispersion across exchanges / mark-index separate legs 등) 만 OI×premium 도메인에서 허용. **53 R-1 graveyards / 5% PASS rate / 4 retired families + 1 advisory caution family (5m microstructure single-domain)** 메타 상태에서 frontier 극소화 인정 강화. /new-paradigm-frontier 2개 연속 dispatch (82 + 83) 모두 BROAD_FALSIFIED — axis stacking does not synthesize alpha (lesson #21). **Day 7 baseline (2026-05-21, 6일 남음) 우선 모드 정식 진입 권고** — paradigm 69 13 sessions 실측 우선, 추가 5m microstructure 도메인 R-1 dispatch는 사용자 명시 승인 시에만 진행. |

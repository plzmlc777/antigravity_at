...
ARMS_DEFAULT = [
    ...
    # ("hifr", ...) — **2026-08-12 종결.** 4사건 -30.6/-19.0/-107.7/+10.1, 평균 -36.78bp.
    # 펀딩률 3bp 이상만 걸러 체결 종목이 16~27개뿐이라 분산이 큰데, 그 분산이
    # 전부 손실 쪽이었다. 백테스트 +24.39bp 는 모멘텀 성분이 섞인 값이었고
    # 라이브에서 재현되지 않았다.
    ...
    # ── 2026-08-11 추가 — 진입시각과 보유시간을 **분리**하기 위한 4팔 ──────────
    # 사건 2회 시점의 지형(롱 7팔):
    #   보유 15분 3팔 평균 -9.98bp / 보유 30분 -5.62 / 보유 60분 3팔 +6.81
    #   보유 60분 안에서 진입만 보면  T-60 -1.42 / **T-30 +19.36** / T-15 +2.50
    # 순위상관은 보유쪽이 더 강하다 (보유 +0.926 vs 진입 -0.721).
    #
    # 문제는 **둘이 얽혀 있다는 것**이다. 기존 팔은 일찍 들어가는 것이 대체로
    # 오래 들고, 늦게 들어가는 것이 짧게 든다. early 만 크게 양수인데 그게
    # "T-30 이라서" 인지 "60분 들어서" 인지 지금 데이터로는 못 가른다.
    #
    # 아래 4팔이 그 매듭을 푼다:
    #   e45 / e20  — 보유 60분 **고정**, 진입만 T-45 / T-20 (봉우리 형태 확인)
    #   e30h30 / e30h90 — 진입 T-30 **고정**, 보유만 30 / 90분 (보유 축 단독 확인)
    # early(T-30/보유60) 가 두 격자의 교차점이라 기준점 역할을 한다.
    ("e45",         -45,  15,  "long",   0.0,  "보유60 고정, 진입 T-45"),
    ("e20",         -20,  40,  "long",   0.0,  "보유60 고정, 진입 T-20"),
    ("e30h30",      -30,   0,  "long",   0.0,  "진입 T-30 고정, 보유 30분"),
    ("e30h90",      -30,  60,  "long",   0.0,  "진입 T-30 고정, 보유 90분"),
]

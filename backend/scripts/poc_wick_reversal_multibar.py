#!/usr/bin/env python3
"""Phase R-1 PoC: Wick Reversal Multi-Bar (Q3 #4).

Q3 #2 wick_reversal POSITIVE 3σ (SOL 3.34σ + AVAX 2.99σ) used SINGLE-bar
wick. Hypothesis: SUSTAINED wick dominance over multiple consecutive bars
indicates more credible liquidation cascade, with reduced random_std (a
random shuffle won't preserve sequences) → potential 4σ+ elevation.

Q3 #3 wick_reversal_volume confirmed §3-H AND-filter degrades signal.
This paradigm avoids §3-H by using SEQUENCE PATTERN (rolling avg over
N bars), not filter. Multi-bar average is a transformation of the wick
metric, not a binary gate stacked on top.

Distinct from prior 51 paradigms:
  - wick_reversal (Q3 #2 POSITIVE): instantaneous single-bar wick
  - This: rolling N-bar mean of wick_frac (smoothed)
  - Mechanism: noise reduction via averaging vs filter selectivity

Entry rule (per symbol, 5m bars):
  1. lwf_meanN = rolling N-bar mean of lower_wick_frac
     uwf_meanN = rolling N-bar mean of upper_wick_frac
  2. prior_ret = close.pct_change(prior_lookback)
  3. lwf_meanN > thresh AND prior_ret < -pm → LONG (sustained lower-wick + drop)
     uwf_meanN > thresh AND prior_ret > +pm → SHORT
  4. Hold hold_bars or stop on SL.

§3-A risk: sustained wick is rarer. R-1 must sweep N ∈ {2, 3, 5} and
threshold to confirm signal exists at moderate sample sizes (not 5-15
trades single-symbol-fit).

Usage:
  python -m scripts.poc_wick_reversal_multibar --symbols SOLUSDT
"""
...
log = logging.getLogger("poc_wick_reversal_multibar")

PARADIGM = "wick_reversal_multibar"
...
def simulate(df: pd.DataFrame, *, n_bars: int, wick_thresh: float,
    ...
    df["lwf_mean"] = df["lower_wick_frac"].rolling(n_bars).mean()
    df["uwf_mean"] = df["upper_wick_frac"].rolling(n_bars).mean()
    df["prior_ret"] = df["close"].pct_change(prior_lookback)
    df = df.dropna(subset=["lwf_mean", "uwf_mean", "prior_ret"])
    ...
    for i in range(1, len(test)):
        ...
        if not in_pos:
            row = test.iloc[i]
            lwf = float(row["lwf_mean"])
            uwf = float(row["uwf_mean"])
...
def main() -> int:
    ...
    p.add_argument("--wick-thresh", type=float, default=0.45)
    ...
    tag = args.tag or (
        f"poc_n{args.n_bars}_wt{args.wick_thresh}_pl{args.prior_lookback}"
    ...
    for sym in args.symbols:
        try:
            ...
            sim = simulate(
                df, n_bars=args.n_bars, wick_thresh=args.wick_thresh,

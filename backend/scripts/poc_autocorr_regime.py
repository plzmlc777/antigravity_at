#!/usr/bin/env python3
"""Phase R-1 PoC: Autocorrelation Regime paradigm.

Hypothesis: rolling lag-1 autocorrelation of 5m log returns identifies the
current return-dependence regime:
  - Positive autocorr (e.g., > +0.10): trending/persistent regime → momentum
    entry (recent direction continues)
  - Negative autocorr (e.g., < -0.10): mean-reverting regime → reversion entry
    (recent direction reverses)
  - Near zero: neutral, no entry

Distinct from all 11 graveyard paradigms:
  - All moment paradigms (mean/std/skew/kurt) are single-distribution shape
  - autocorr is a 2-point time-dependence statistic — orthogonal dimension
  - Auto-regime switching: same symbol uses different strategy by current
    autocorr sign, instead of one fixed direction

Pipeline:
  1. Load 1m → resample to 5m bars.
  2. Compute rolling AUTOCORR_WINDOW lag-1 autocorrelation of log returns.
  3. Compute REGIME_WINDOW percentile rank of autocorr (or use absolute thresholds).
  4. Compute DIR_LOOKBACK-bar return as direction signal.
  5. Entry:
     - autocorr > +TREND_THRESH AND dir_ret > 0 → LONG (momentum)
     - autocorr > +TREND_THRESH AND dir_ret < 0 → SHORT (momentum)
     - autocorr < -REV_THRESH AND dir_ret > 0 → SHORT (reversion: fade up move)
     - autocorr < -REV_THRESH AND dir_ret < 0 → LONG (reversion: fade down move)
  6. Exit at HOLD bars or SL.

Usage:
  python -m scripts.poc_autocorr_regime --symbols SOLUSDT
"""
from __future__ import annotations

import argparse
import json
import logging
import math
import sys
from datetime import datetime, timezone
from pathlib import Path

import numpy as np
import pandas as pd
from sqlalchemy import text

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT))

from app.db.session import SessionLocal  # noqa: E402

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("poc_autocorr_regime")

PARADIGM = "autocorr_regime"
OUT_DIR = ROOT / "runs" / "research_track" / PARADIGM


def load_ohlcv_5m(symbol: str) -> pd.DataFrame:
    s = SessionLocal()
    try:
        df = pd.read_sql(
            text("""
                SELECT timestamp AS ts, close
                FROM ohlcv WHERE symbol=:sym AND time_frame='1m'
                ORDER BY timestamp
            """),
            s.connection(),
            params={"sym": symbol},
            parse_dates=["ts"],
        )
    finally:
        s.close()
    if df.empty:
        raise ValueError(f"No 1m ohlcv for {symbol}")
    df = df.set_index("ts")["close"].astype(float)
    df_5m = df.resample("5min", label="right", closed="right").last().dropna().to_frame("close")
    log.info("Loaded %s 5m: %d bars (%s → %s)",
             symbol, len(df_5m), df_5m.index[0], df_5m.index[-1])
    return df_5m


def _rolling_autocorr(s: pd.Series, window: int, lag: int = 1) -> pd.Series:
    """Compute rolling lag-N autocorrelation using rolling.corr (vectorized).

    Equivalent to s.rolling(W).apply(lambda x: pd.Series(x).autocorr(lag))
    but ~100x faster.
    """
    return s.rolling(window).corr(s.shift(lag))


def simulate_autocorr_regime(df: pd.DataFrame, *, autocorr_window: int, lag: int,
                              trend_thresh: float, rev_thresh: float,
                              dir_lookback: int, hold_bars: int, sl_pct: float,
                              fee_rate: float, capital: float, train_frac: float,
                              skip_neutral: bool = True,
                              regime_filter: str = "both") -> dict:
    """regime_filter: 'both' / 'trend_only' / 'rev_only'"""
    df = df.copy()
    df["ret"] = np.log(df["close"] / df["close"].shift(1))
    df["acorr"] = _rolling_autocorr(df["ret"], autocorr_window, lag=lag)
    df["dir_ret"] = df["close"].pct_change(dir_lookback).shift(1)
    df = df.dropna(subset=["acorr", "dir_ret"])

    n = len(df)
    if n < 1000:
        return {"error": f"too few bars ({n})", "n_trades": 0}
    split = int(n * train_frac)
    test = df.iloc[split:].copy()

    equity = capital
    equity_curve = [(test.index[0], equity)]
    trades: list[dict] = []
    in_pos = False; side = 0; entry_px = 0.0; bars_held = 0
    entry_ts = ""; entry_acorr = 0.0; target_hold = 0; entry_regime = ""

    n_trend = 0; n_rev = 0  # entry counts by regime

    for i in range(1, len(test)):
        ts = test.index[i]
        px = float(test["close"].iloc[i])
        if in_pos:
            bars_held += 1
            price_pnl = side * (px - entry_px) / entry_px
            unrealized = price_pnl
            exit_reason = None
            if unrealized < -sl_pct:
                exit_reason = "sl"
            elif bars_held >= target_hold:
                exit_reason = "time"
            if exit_reason:
                ret_pct = unrealized - 2 * fee_rate
                equity *= (1 + ret_pct)
                trades.append({
                    "entry_ts": entry_ts, "exit_ts": str(ts),
                    "side": side, "entry_acorr": entry_acorr,
                    "regime": entry_regime,
                    "return_pct": ret_pct, "exit_reason": exit_reason,
                    "bars_held": bars_held,
                })
                in_pos = False; side = 0; bars_held = 0

        if not in_pos:
            row = test.iloc[i]
            ac = float(row["acorr"]); dr = float(row["dir_ret"])
            if not math.isnan(ac) and not math.isnan(dr) and dr != 0:
                if ac > trend_thresh and regime_filter in ("both", "trend_only"):
                    side = 1 if dr > 0 else -1
                    entry_regime = "trend"
                    n_trend += 1
                elif ac < -rev_thresh and regime_filter in ("both", "rev_only"):
                    side = -1 if dr > 0 else 1
                    entry_regime = "rev"
                    n_rev += 1
                else:
                    if skip_neutral:
                        continue
                    side = 0  # don't enter
                if side != 0:
                    in_pos = True
                    entry_px = px; entry_ts = str(ts); entry_acorr = ac
                    bars_held = 0; target_hold = hold_bars
        equity_curve.append((ts, equity))

    bh_pct = (test["close"].iloc[-1] / test["close"].iloc[0]) - 1
    total_return_pct = (equity / capital) - 1
    alpha_pct = (total_return_pct - bh_pct) * 100

    eq = np.array([e for _, e in equity_curve])
    peak = np.maximum.accumulate(eq)
    dd = (peak - eq) / peak
    max_dd_pct = float(dd.max() * 100) if len(dd) else 0.0

    if trades:
        rs = np.array([t["return_pct"] for t in trades])
        mu = rs.mean(); sd = rs.std(ddof=1) if len(rs) > 1 else 0.0
        oos_seconds = (test.index[-1] - test.index[0]).total_seconds()
        trades_per_year = (len(trades) / oos_seconds * 31536000.0
                           if oos_seconds > 0 else 0)
        sharpe_ann = (float(mu / sd * math.sqrt(max(trades_per_year, 1)))
                      if sd > 0 else 0.0)
        wins = rs[rs > 0]; losses = rs[rs < 0]
        win_rate_pct = float(len(wins) / len(rs) * 100)
        gw = float(wins.sum()) if len(wins) else 0.0
        gl = float(-losses.sum()) if len(losses) else 0.0
        profit_factor = (float(gw / gl) if gl > 0
                         else (float("inf") if gw > 0 else 0.0))
        # split metrics by regime
        trend_rs = np.array([t["return_pct"] for t in trades if t["regime"] == "trend"])
        rev_rs = np.array([t["return_pct"] for t in trades if t["regime"] == "rev"])
        trend_alpha = float(trend_rs.sum() * 100) if len(trend_rs) else 0.0
        rev_alpha = float(rev_rs.sum() * 100) if len(rev_rs) else 0.0
    else:
        sharpe_ann = win_rate_pct = profit_factor = 0.0
        trend_alpha = rev_alpha = 0.0

    oos_days = int((test.index[-1] - test.index[0]).total_seconds() // 86400)

    return {
        "n_trades": len(trades),
        "n_trend_entries": n_trend,
        "n_rev_entries": n_rev,
        "alpha_pct": round(alpha_pct, 2),
        "trend_alpha_sum": round(trend_alpha, 2),
        "rev_alpha_sum": round(rev_alpha, 2),
        "total_return_pct": round(total_return_pct * 100, 2),
        "buy_hold_pct": round(bh_pct * 100, 2),
        "sharpe_ann": round(sharpe_ann, 3),
        "max_dd_pct": round(max_dd_pct, 2),
        "win_rate_pct": round(win_rate_pct, 2),
        "profit_factor": (round(profit_factor, 3)
                          if profit_factor != float("inf") else "inf"),
        "oos_days": oos_days,
    }


def main() -> int:
    p = argparse.ArgumentParser()
    p.add_argument("--symbols", nargs="+", default=["SOLUSDT"])
    p.add_argument("--autocorr-window", type=int, default=288)   # 24h window
    p.add_argument("--lag", type=int, default=1)
    p.add_argument("--trend-thresh", type=float, default=0.10)
    p.add_argument("--rev-thresh", type=float, default=0.10)
    p.add_argument("--dir-lookback", type=int, default=12)        # 1h direction
    p.add_argument("--hold-bars", type=int, default=24)           # 2h hold
    p.add_argument("--sl-pct", type=float, default=0.02)
    p.add_argument("--fee-rate", type=float, default=0.0004)
    p.add_argument("--capital", type=float, default=1_000_000.0)
    p.add_argument("--train-frac", type=float, default=0.5)
    p.add_argument("--regime-filter", choices=["both", "trend_only", "rev_only"],
                   default="both")
    p.add_argument("--tag", default=None)
    args = p.parse_args()

    tag = args.tag or (
        f"poc_aw{args.autocorr_window}_l{args.lag}_t{args.trend_thresh}"
        f"_r{args.rev_thresh}_dl{args.dir_lookback}_h{args.hold_bars}"
    )
    OUT_DIR.mkdir(parents=True, exist_ok=True)

    rows = []
    for sym in args.symbols:
        try:
            df = load_ohlcv_5m(sym)
            sim = simulate_autocorr_regime(
                df, autocorr_window=args.autocorr_window, lag=args.lag,
                trend_thresh=args.trend_thresh, rev_thresh=args.rev_thresh,
                dir_lookback=args.dir_lookback, hold_bars=args.hold_bars,
                sl_pct=args.sl_pct, fee_rate=args.fee_rate,
                capital=args.capital, train_frac=args.train_frac,
                regime_filter=args.regime_filter,
            )
            sim["symbol"] = sym
            rows.append(sim)
            log.info("%s — alpha=%.2f sharpe=%.2f mdd=%.1f wr=%.1f pf=%s "
                     "trades=%d (T=%d R=%d) trend_a=%.1f rev_a=%.1f",
                     sym, sim.get("alpha_pct", 0), sim.get("sharpe_ann", 0),
                     sim.get("max_dd_pct", 0), sim.get("win_rate_pct", 0),
                     sim.get("profit_factor", 0), sim["n_trades"],
                     sim.get("n_trend_entries", 0), sim.get("n_rev_entries", 0),
                     sim.get("trend_alpha_sum", 0), sim.get("rev_alpha_sum", 0))
        except Exception as exc:
            log.exception("Failed %s: %s", sym, exc)

    df_out = pd.DataFrame(rows)
    cols = ["symbol", "n_trades", "n_trend_entries", "n_rev_entries",
            "alpha_pct", "trend_alpha_sum", "rev_alpha_sum",
            "total_return_pct", "buy_hold_pct", "sharpe_ann", "max_dd_pct",
            "win_rate_pct", "profit_factor", "oos_days"]
    df_out = df_out.reindex(columns=[c for c in cols if c in df_out.columns])
    out_csv = OUT_DIR / f"{tag}__per_symbol.csv"
    df_out.to_csv(out_csv, index=False)

    if len(rows) > 0:
        agg = {
            "spec_name": tag, "n_symbols": len(rows),
            "alpha_pct_mean": round(float(df_out["alpha_pct"].mean()), 2),
            "alpha_pos_count": int((df_out["alpha_pct"] > 0).sum()),
            "sharpe_mean": round(float(df_out["sharpe_ann"].mean()), 3),
            "sharpe_pos_count": int((df_out["sharpe_ann"] > 0).sum()),
            "trend_alpha_mean": round(float(df_out["trend_alpha_sum"].mean()), 2),
            "rev_alpha_mean": round(float(df_out["rev_alpha_sum"].mean()), 2),
            "trades_total": int(df_out["n_trades"].sum()),
        }
        out_meta = {
            "paradigm": PARADIGM, "phase": "R-1_PoC", "spec_name": tag,
            "evaluated_at": datetime.now(tz=timezone.utc).isoformat(),
            "config": vars(args), "aggregate": agg, "per_symbol": rows,
        }
        out_meta_path = OUT_DIR / f"{tag}__metrics.json"
        out_meta_path.write_text(json.dumps(out_meta, indent=2, default=str))
        print("\n=== Aggregate ===")
        print(json.dumps(agg, indent=2))
    return 0


if __name__ == "__main__":
    sys.exit(main())

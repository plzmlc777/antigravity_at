#!/usr/bin/env python3
"""Phase R-3 robustness for book_depth_imbalance_zscore (n=200 perm test).

Permutation: shuffle daily imbalance series; keep close (price) intact.
Real signal lies in joint timing of imbalance z-extreme and subsequent
price direction. Shuffling imbalance destroys time-dependence of LOB
pressure while preserving price autocorrelation.
...
from scripts.poc_book_depth_imbalance_zscore import (  # noqa: E402
    PARADIGM, OUT_DIR, load_close_1d, load_imbalance_1d, simulate,
)

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("poc_book_depth_imbalance_zscore_r3")


def permutation_test(close: pd.Series, imbalance: pd.Series, n_iter: int,
                     real_alpha: float, **rule_params) -> dict:
    rng = np.random.default_rng(seed=42)
    base_imb = imbalance.values.astype(float)
    random_alphas = []
    for it in range(n_iter):
        shuf = rng.permutation(base_imb)
        shuf_series = pd.Series(shuf, index=imbalance.index)
...
def main() -> int:
    ...
    p.add_argument("--symbols", nargs="+",
                   default=["SOLUSDT", "ETHUSDT", "BTCUSDT", "DOGEUSDT"])
    ...
    for sym in args.symbols:
        try:
            ...
            real = simulate(close, imb, train_frac=0.5, **rule_params)
            ...
            perm = permutation_test(close, imb, args.n_iter_perm, real_alpha, **rule_params)
            out = {

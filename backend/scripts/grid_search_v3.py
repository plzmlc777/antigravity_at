"""Grid search v3 — premium index source added.

Tests if Premium Index (futures-spot basis) provides decisive signal
that funding rate (which failed) doesn't.

Search space:
  - 14 symbols (all)
  - 6 source combos including P (premium)
  - 2 fwd, 2 policies, 2 thresholds
  → 14 × 6 × 2 × 2 × 2 = 672 conditions
"""
...
# Combos with P (premium) as new dimension. Keep best previous combos as anchors.
SOURCE_COMBOS = [
    ("P",),                          # premium only
    ("S", "P"),
    ("T", "P"),
    ("O", "P"),
    ("T", "O", "P"),                 # boost DOGE winner
    ("S", "T", "P"),                 # boost SOL winner
]
FORWARD_BARS = [5, 10]
POLICIES = ["long_only", "long_short"]
THRESHOLDS = [0.003, 0.005]
...
def load_data(sym):
    ...
    return df_eval, signals, metrics, premium


def attach_combo(feat_pat, df_eval, metrics, premium, combo, eval_freq_min=1440):
    out = feat_pat
...
def evaluate(sym, df_eval, signals, metrics, premium, combo, fwd, policy, threshold):
    ...
    feat = attach_combo(feat_pat, df_eval, metrics, premium, combo)
...
def main():
    ...
    print(f"Grid search v3 — Premium index source.  {n_total} conditions to test.")

    results = []
    for sym in SYMBOLS:
        try:
            df_eval, signals, metrics, premium = load_data(sym)
            if signals is None or premium is None:
                print(f"[{sym}] SKIP — missing signals/premium")
                continue
        ...
        for combo, fwd, policy, thr in itertools.product(SOURCE_COMBOS, FORWARD_BARS, POLICIES, THRESHOLDS):
            try:
                r = evaluate(sym, df_eval, signals, metrics, premium, combo, fwd, policy, thr)
            except Exception:
                continue
    ...
    print("\n>>> SOL-equivalent (gate AND alpha_vf ≥+27pts):")
    eq = df_sorted[(df_sorted["sign"] >= 0.60) & (df_sorted["binom_p"] < 0.01) & (df_sorted["alpha_vf_pts"] >= SOL_ALPHA_BASELINE)]
    print(f"    {len(eq)} conditions")
    if len(eq) > 0:
        print(eq[["symbol", "combo", "fwd", "policy", "threshold", "sign", "binom_p", "alpha_pts", "alpha_vf_pts", "trades"]].to_string(index=False))

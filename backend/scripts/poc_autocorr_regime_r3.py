#!/usr/bin/env python3
"""Phase R-3 robustness for autocorr_regime (n=200 perm test, top 3 candidates)."""
...
from scripts.poc_autocorr_regime import (  # noqa: E402
    PARADIGM, OUT_DIR, load_ohlcv_5m, simulate_autocorr_regime,
)

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("poc_autocorr_regime_r3")


def permutation_test(df: pd.DataFrame, n_iter: int, real_alpha: float,
                     **rule_params) -> dict:
    """Shuffle 5m return series — destroys autocorrelation while preserving
    ...
    for it in range(n_iter):
        ...
        sim = simulate_autocorr_regime(shuf, train_frac=0.5, **rule_params)
        if sim.get("error"):
...
def main() -> int:
    ...
    p.add_argument("--symbols", nargs="+", default=["DOGEUSDT", "ETCUSDT", "SOLUSDT"])
    p.add_argument("--autocorr-window", type=int, default=288)
    p.add_argument("--lag", type=int, default=1)
    p.add_argument("--trend-thresh", type=float, default=0.20)
    p.add_argument("--rev-thresh", type=float, default=0.20)
    ...
    rule_params = dict(autocorr_window=args.autocorr_window, lag=args.lag,
                       ...
                       capital=args.capital, regime_filter=args.regime_filter)
    ...
    for sym in args.symbols:
        ...
        real_sim = simulate_autocorr_regime(df, train_frac=args.train_frac,
                                              **rule_params)

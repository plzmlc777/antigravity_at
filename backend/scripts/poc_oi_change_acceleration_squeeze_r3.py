#!/usr/bin/env python3
"""Phase R-3 robustness for oi_change_acceleration_squeeze (n=200 perm test).

Permutation: shuffle OI series (preserve price). Tests if OI acceleration
timing predicts price moves vs random.
"""
...
from scripts.poc_oi_change_acceleration_squeeze import (  # noqa: E402
    PARADIGM, OUT_DIR, load_close_1d, load_oi_1d, simulate,
)

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("poc_oi_change_acceleration_squeeze_r3")


def permutation_test(close, oi, n_iter, real_alpha, **rule_params):
    rng = np.random.default_rng(seed=42)
    base = oi.values.astype(float)
    random_alphas = []
    for it in range(n_iter):
        shuf = rng.permutation(base)
        shuf_series = pd.Series(shuf, index=oi.index)
...
def main() -> int:
    ...
    p.add_argument("--symbols", nargs="+",
                   default=["ETCUSDT", "LINKUSDT", "SOLUSDT", "LDOUSDT"])
    ...
    p.add_argument("--mode", default="follow_accel")
    ...
    for sym in args.symbols:
        try:
            ...
            oi = load_oi_1d(sym)
            real = simulate(close, oi, train_frac=0.5, **rule_params)
            ...
            perm = permutation_test(close, oi, args.n_iter_perm, real_alpha,
                                    **rule_params)

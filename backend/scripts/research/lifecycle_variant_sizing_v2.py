...
왜 v2 인가 (2026-08-12)
  ...
  그래서 v2 는 두 가지를 바꾼다:
    ...
    (b) **회귀 검사를 먼저 돌린다.** 기록된 옛 숫자와 비교하지 않는다 — 코호트가
        자라기 때문이다(일별 백필로 30일치를 채운 종목이 새로 편입되어 129 → 131).
        대신 **원본 `BASE.run()` 을 지금 같은 코호트로 실행**해서 v2 의
        base × cap 0.20 × 1배와 대조한다. 데이터가 동일하므로 차이가 나면
        그건 **코드 차이**다. 불일치면 즉시 중단한다.
...
# 회귀 허용 오차 — 원본 `BASE.run()` 과의 차이. 같은 코호트이므로 0 이어야 한다.
TOL = {"final": 0.01, "mdd": 0.01, "taken": 0, "starved": 0}
...
def attach_volume(db, cohort: list) -> int:
    """코호트 `daily` 에 일별 volume 을 붙인다.

    원본 `load_daily` 는 OHLC 만 싣는다(volume 컬럼 없음). 그래서 조기청산
    변형의 vol_cliff 조건이 통째로 건너뛰어져 **발동률 0%** 가 됐다 —
    earlyexit_d7/d14 결과가 base 와 소수점까지 같았던 원인이다.
    (251건 거래단위 백테스트에서는 d7 이 58.6% 발동했다.)

    원본을 고치지 않는다 — 원본은 회귀 검사의 기준이므로 건드리면 기준 자체가
    흔들린다. 대신 여기서 volume 만 따로 읽어 병합한다.
    반환: volume 을 붙인 종목 수.
    """
    ...
    for c in cohort:
        rows = db.execute(text(
            "SELECT timestamp, volume FROM ohlcv WHERE symbol=:s "
            "AND time_frame='1m' ORDER BY timestamp"), {"s": c["symbol"]}).fetchall()
        if not rows:
            continue
        v = pd.Series({pd.Timestamp(t): float(x or 0) for t, x in rows}).sort_index()
        dv = v.resample("1D").sum()
        c["daily"] = c["daily"].assign(
            volume=dv.reindex(c["daily"].index).fillna(0.0).values)
        n += 1
    return n
...
def run_pf(trades: list, cap_frac: float, lev: int, initial: float) -> dict:
    ...
    return {"final": round(wallet, 2), "ret_pct": round((wallet / initial - 1) * 100, 2),
            "pnl_total": round(sum(pn), 2) if pn else 0.0,
            "pnl_per_trade": round(float(np.mean(pn)), 2) if pn else 0.0,
            "win_rate": round(100 * len(wins) / max(len(pn), 1), 1),
...
def main() -> int:
    ...
    # 조기청산 판정에 필요한 volume 을 붙인다 (원본은 OHLC 만 싣는다)
    db2 = SessionLocal()
    try:
        nv = attach_volume(db2, cohort)
    finally:
        db2.close()
    log.info("volume 부착 %d/%d종목", nv, len(cohort))
    if nv < len(cohort) * 0.9:
        log.error("volume 부착률 %.0f%% — 조기청산 판정 불가, 중단", 100 * nv / len(cohort))
        return 1

    # ── ① 회귀 검사 — 재현 못 하면 중단 ────────────────────────────
    #
    # 코호트를 **기준값 생성 시점(2026-08-08)** 으로 되돌린다. `build_cohort` 는
    # "상장 후 30~365일 경과" 종목만 담으므로, 그 뒤 상장분이 30일 요건을 채우면
    # 코호트가 자란다(129 → 131). 그 상태로 검사하면 코드가 같아도 실패한다.
    # 기준 시점에 적격이던 종목만 남겨야 **코드 동일성**을 검증할 수 있다.
    # 원본을 지금 같은 코호트로 돌려 기준을 만든다 (기록값이 아니라 실행값).
    ref_run = BASE.run(cohort, 0.20)
    base_tr = []
    for c in cohort:
    ...
    log.info("회귀 검사 (원본 대조, 같은 코호트 %d건)", len(cohort))
    log.info("  원본: final %.2f / mdd %.2f / taken %d / starved %d",
             ref_run["final_wallet"], ref_run["mdd_pct"],
             ref_run["n_taken"], ref_run["n_starved"])
    log.info("  v2  : final %.2f / mdd %.2f / taken %d / starved %d",
             chk["final"], chk["mdd_pct"], chk["taken"], chk["starved"])
    bad = []
    if abs(chk["final"] - ref_run["final_wallet"]) > TOL["final"]:
        bad.append(f"final {chk['final']} vs {ref_run['final_wallet']}")
    if abs(chk["mdd_pct"] - ref_run["mdd_pct"]) > TOL["mdd"]:
        bad.append(f"mdd {chk['mdd_pct']} vs {ref_run['mdd_pct']}")
    if chk["taken"] != ref_run["n_taken"]:
        bad.append(f"taken {chk['taken']} vs {ref_run['n_taken']}")
    if chk["starved"] != ref_run["n_starved"]:
        bad.append(f"starved {chk['starved']} vs {ref_run['n_starved']}")
    if bad:
        log.error("**회귀 검사 실패 — 중단**: %s", " | ".join(bad))
        log.error("원본과 같은 데이터인데 결과가 다르다 = v2 코드 결함.")
        return 1
    ...
    print("-" * 150)
    print(f"  {'신호변형':<15}{'사이징':<11}{'포착%':>7}{'MDD%':>8}{'최악$':>9}"
          f"{'최고$':>9}{'승률%':>7}{'PF':>6}{'거래당$':>9}{'총손익$':>10}"
          f"{'최종$':>10}{'수익%':>9}{'SL%':>6}{'조기%':>6}{'잡음':>5}{'놓침':>5}")
    print("-" * 150)
    for vname, _, _ in VARIANTS:
        for sname, _, _ in SIZINGS:
            r = res[f"{vname}|{sname}"]
            pf = f"{r['pf']:.2f}" if r.get("pf") else "—"
            print(f"  {vname:<15}{sname:<11}{r['capture']:>7.1f}{r['mdd_pct']:>8.2f}"
                  f"{r['worst']:>9.2f}{r['best']:>9.2f}{r['win_rate']:>7.1f}{pf:>6}"
                  f"{r['pnl_per_trade']:>9.2f}{r['pnl_total']:>10.2f}"
                  f"{r['final']:>10,.2f}{r['ret_pct']:>+9.1f}"
                  f"{r['sl_pct']:>6.1f}{r['early_pct']:>6.1f}"
                  f"{r['taken']:>5}{r['starved']:>5}")
        print()
    print("-" * 150)
    print("  ** 사이징 고정 — 손익 기준 순위 (거래당 손익) **")
    for sname, _, _ in SIZINGS:
        row = [(v, res[f"{v}|{sname}"]) for v, _, _ in VARIANTS]
        row.sort(key=lambda x: -x[1]["pnl_per_trade"])
        print(f"    [{sname}]  " + "  >  ".join(
            f"{v}(${r['pnl_per_trade']:.2f}/PF{r['pf'] if r['pf'] else '—'})" for v, r in row))
    print("-" * 150)
    ...
    json.dump({"cohort": len(cohort), "initial": initial,
               "regression_check": chk, "reference_run": ref_run, "results": res},

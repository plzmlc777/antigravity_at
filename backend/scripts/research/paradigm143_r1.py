"""Paradigm 143 R-1: alt_taker_buy_quote_vol_percentile_rank_directional_8h.

Hypothesis
----------
Per-symbol 4h bar taker_buy_quote_volume / quote_volume imbalance ratio. Compute
30d (180 bars × 4h) rolling percentile rank (distribution-agnostic, bounded
[0,1], no mean/std assumptions). When extreme regime triggers:
  - percentile_rank > 0.95 (top 5% aggressive USD buy) → 8h LONG continuation
  - percentile_rank < 0.05 (bottom 5% aggressive USD sell) → 8h SHORT continuation

Differentiation from paradigm 142-v2 (Lesson #44 amendment 26th xref)
---------------------------------------------------------------------
- Trigger statistic: 30d rolling **percentile rank** (vs z-score), avoids
  distribution asymmetry trap (Lesson #55 candidate prescription).
- Primary hold: 8h (2 bars × 4h) — uses paradigm 142-v2 B_focus SHORT 12h
  sigex +3.43 off-primary hint as midpoint candidate (vs primary 4h fail at +1.82).
- Same 12-col klines cache substrate (zero new infra).
- Lesson #40 N/A: percentile rank ∈ [0,1] structurally attainable both tails.

4-quadrant SNT (Lesson #19 mandatory for symmetric joint-trigger)
-----------------------------------------------------------------
  - A focus: rank > 0.95 × 8h LONG (primary)
  - A mirror: rank > 0.95 × 8h SHORT
  - B focus: rank < 0.05 × 8h SHORT (mechanism mirror)
  - B mirror: rank < 0.05 × 8h LONG

Lesson #57 2nd dogfood
---------------------
- If BROAD_FALSIFIED → quote_vol axis 4h+ directional family Tier 4 retire
  candidate formal elevation (paradigm 142-v2 + 143 both BROAD_FALSIFIED).
- If PASS → percentile rank distribution-agnostic prescription validated,
  Lesson #55 candidate 3rd dogfood TRUE POSITIVE.
"""
...
log = logging.getLogger("paradigm143_r1")
...
HOLD_BARS_PRIMARY = 2  # 2 × 4h = 8h (paradigm 142-v2 B_focus 12h hint midpoint)
HOLD_SWEEP_BARS = [1, 2, 3]  # 4h / 8h / 12h
ROLLING_BARS = 180  # 30d × 6 bars/day
PCT_TOP_THRESHOLD = 0.95  # top 5% aggressive buy
PCT_BOT_THRESHOLD = 0.05  # bottom 5% aggressive sell
...
def compute_percentile_rank_panel(panel: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
    """Return same panel with extra columns: imbalance, pct_rank (30d rolling)."""
    out = {}
    for sym, df in panel.items():
        ...
        # 30d rolling percentile rank, shift(1) to avoid look-ahead
        # rank within each rolling window: position of current value / window size
        # use rolling apply with rank — but apply is slow; vectorize via percentile formula
        # We compute rank-of-last via rolling apply with raw=True for speed
        def _last_rank(x):
            # rank of last element in window (0..1 scale, average for ties)
            n = len(x)
            if n < 2:
                return np.nan
            last = x[-1]
            # number of values strictly less + 0.5 * number of ties (excluding self)
            less = np.sum(x[:-1] < last)
            ties = np.sum(x[:-1] == last)
            rank = (less + 0.5 * ties) / (n - 1)
            return rank
        # min_periods=60 like paradigm 142-v2 stability floor
        d["pct_rank"] = (
            d["imbalance"]
            .rolling(ROLLING_BARS, min_periods=60)
            .apply(_last_rank, raw=True)
            .shift(1)  # ensure no look-ahead: rank computed up to t-1 used to trigger at t
        )
        out[sym] = d
    return out
...
def gather_events(panel_z: Dict[str, pd.DataFrame], side: str,
    ...
    """Build event dataframe: trigger bars matching side (pos/top or neg/bottom), with
    forward close-to-close return over `hold_bars`.

    side='pos' → pct_rank > PCT_TOP_THRESHOLD;
    side='neg' → pct_rank < PCT_BOT_THRESHOLD.
    """
    rows: List[Dict] = []
    for sym, d in panel_z.items():
        if side == "pos":
            mask = d["pct_rank"] > PCT_TOP_THRESHOLD
        else:
            mask = d["pct_rank"] < PCT_BOT_THRESHOLD
...
def main() -> int:
    t_start = time.time()
    log.info("paradigm 143 R-1 starting at %s", pd.Timestamp.utcnow())
    panel = load_panel()
    if not panel:
    ...
    log.info("computing 30d rolling percentile rank (may take ~30-60s) ...")
    panel_z = compute_percentile_rank_panel(panel)
    log.info("percentile rank done")
    ...
    log.info("trigger events: pos (rank>%.2f) n=%d, neg (rank<%.2f) n=%d",
             PCT_TOP_THRESHOLD, len(events_pos), PCT_BOT_THRESHOLD, len(events_neg))

    # 4-quadrant SNT (primary hold)
    log.info("=== 4-quadrant SNT (Lesson #19) primary hold %dh ===", HOLD_BARS_PRIMARY * 4)
    ...
    metrics = {
        ...
        "config": {
            ...
            "pct_top_threshold": PCT_TOP_THRESHOLD,
            "pct_bot_threshold": PCT_BOT_THRESHOLD,
        ...
        "family_distinct_paradigms": {
            "paradigm_72_taker_buy_vol_5m_BROAD_FALSIFIED": "distinct: 4h frame + percentile rank vs raw 5m absolute",
            "paradigm_127_128_volume_burst_R5_LIVE_30m": "distinct: continuous percentile vs burst spike + 4h/8h vs 30m",
            "paradigm_140_CVD_ratio": "distinct: percentile rank quote-imbalance vs raw CVD cumulative",
            "paradigm_142v2_z_score_4h": "distinct: percentile rank distribution-agnostic + 8h primary hold vs z 4h primary",
            "funding_family_22_132_138_141_137": "distinct axis: taker-side action vs perp financing",
            "dna_overlap_dim": "trigger statistic only (1/6); domain + tf + primary hold + normalization scheme all distinct",
        },
        "lessons_applied": {
            "lesson_11_sample_density": "PASS expected per-cell ~344/quarter",
            "lesson_19_4quadrant_SNT_mandatory": "applied (joint-trigger symmetric)",
            "lesson_30_data_window_ratio": "1.00 uniform PASS",
            "lesson_40_structural_threshold_feasibility": "N/A - percentile rank [0,1] bounded both tails attainable",
            "lesson_39_subclass_AB_detection": "see lesson39_subclass_detection",
            "lesson_44_amendment_26th_xref": "see family_distinct_paradigms",
            "lesson_45_no_hmm_unsupervised": "compliant - deterministic percentile rank",
            ...
            "lesson_55_candidate_prescription_dogfood": "percentile rank substitution for z-score (3rd dogfood opportunity)",
            "lesson_57_dogfood_target": "quote_vol axis 4h+ family Tier 4 retire eligible if BROAD_FALSIFIED (2nd dogfood)",
        },
    }

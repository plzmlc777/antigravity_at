"""
paradigm 181 R-1: per-sym 30d return z-score continuous-weighted long-only daily-rebal
...
- IR / tracking error
- Per-sym contribution
- Quarter-by-quarter (9 quarters)
- Turnover + fee drag
- Position util empirical (avg active syms / weight distribution)
- Lesson #20 4-cond per-sym Concentration + Temporal robustness
- Permutation null: shuffle daily z-vector across time, recompute portfolio Sharpe

Verdict tree:
- PORTFOLIO_ALPHA_INSIGNIFICANT — alpha sharpe < benchmark sharpe + 0.5
- BROAD_FALSIFIED_FEE_FLOOR — gross alpha positive but net < 0
- NARROW_SCOPE_LIFE_CHANGING_FAIL — alpha PASS but 4-dim FAIL
- PASS_R1_FULL — alpha sharpe PASS + 4-cond ALL PASS
...
OUT_DIR = Path("/home/hcpark/antigravity/backend/runs/research_track/alt_per_sym_30d_return_z_continuous_weighted_long_only_position_size_daily_rebal")
...
Z_FLOOR = 0.5
Z_CAP = 3.0
FEE_BP_ONE_WAY = 8.0  # one-way 8bp (round-trip 16bp)
...
def load_daily_returns():
    ...
    # Drop rows where any sym has NaN
...
def compute_z_scores(df_close: pd.DataFrame, return_window: int, z_window: int) -> pd.DataFrame:
    """Per-sym 30d rolling return then 90d rolling z-score of that return."""
...
def compute_weights(z: pd.DataFrame, z_floor: float, z_cap: float) -> pd.DataFrame:
    """Position size = clip(z, z_floor, z_cap) where z >= z_floor; else 0.
    Then normalize so sum-of-weights ≤ 1.0 (cash residual if total < 1)."""
    # Raw weight per sym
    w_raw = z.clip(lower=z_floor, upper=z_cap).where(z >= z_floor, 0.0)
    # Sum across syms per day
    row_sum = w_raw.sum(axis=1)
    # Normalize: if row_sum > N_active * z_cap, scale down. Use cap-normalize so sum ≤ 1.
    # We use absolute scale: w_i = w_raw_i / max(sum, 1.0) — when few active syms, cash residual; when many active, scaled
    scale = row_sum.where(row_sum > 0, 1.0)
    # Treat scale floor at sum=N_active for full-investment, but conservative: scale = max(row_sum, 1.0)
...
def simulate_portfolio(weights: pd.DataFrame, returns: pd.DataFrame, fee_bp_one_way: float):
    """Daily-rebal portfolio: ret_t = sum(w_{t-1} × r_t) - turnover_cost.
    weights: index = day, columns = syms. Apply weights at day t to returns at day t+1 (lag).
    Turnover at day t = sum(|w_t - w_{t-1}|). Fee = turnover × fee_bp_one_way / 10000.
    """
    # Align
    w = weights.reindex(returns.index).fillna(0.0)
    # Lag weights by 1 day (signal computed at close of t-1, applied to r_t)
    w_lag = w.shift(1).fillna(0.0)
    # Per-day gross return
    gross_ret = (w_lag * returns).sum(axis=1)
    # Turnover (rebalance from w_{t-1} -> w_t happens at close of t-1; applied to t+1)
    # For fee accounting: turnover at close of t-1 incurs fee on day t boundary
    turnover = w.diff().abs().sum(axis=1).fillna(0.0)
    fee_per_day = turnover * (fee_bp_one_way / 10000.0)
    # Apply fee on the day weights change (i.e., the lag boundary). For simplicity attribute fee to day t.
...
def util_diagnostics(w_lag: pd.DataFrame):
    """Lesson #71 corollary path C util measurement."""
    # Active syms per day
    active_per_day = (w_lag > 0).sum(axis=1)
    # Total weight per day (capital deployed)
...
def permutation_test(weights: pd.DataFrame, returns: pd.DataFrame, fee_bp: float, n_perm: int, seed: int):
    """Shuffle weights across time (preserving cross-sectional structure per day).
    Null: portfolio Sharpe under random temporal alignment of weight rows.
    """
    ...
    for i in range(n_perm):
        ...
        w_perm.index = weights.index  # restore index
...
def four_cond_audit(metrics_port, metrics_eq, util, n_days, quarter_breakdown_dict, per_sym, perm_result):
    """Lesson #20 4-cond + life-changing 4-dim."""
    # Cond 1: three-gate (sharpe excess + ci proxy + perm_p)
    ...
    # Cond 2: Concentration — % syms with positive contribution
    syms_positive = sum(1 for s in per_sym.values() if s["positive"])
    syms_pos_ratio = syms_positive / len(per_sym)
    cond2_pass = syms_pos_ratio >= 0.5  # 7/14+
    # Cond 3: Temporal robustness — quarters positive
    ...
    trades_per_year = n_days / (n_days / 365.0) if n_days > 0 else 0  # daily rebal so ~365/yr per sym * active
    # Effective trades/yr at portfolio level = rebalance freq * avg active syms
    ...
    dim4_pass = {
        ...
        "per_trade_edge_ge_2pct": per_trade_edge_bp >= 200,  # 2% = 200bp
...
def verdict_tree(audit, metrics_port, util):
    ...
    # cond1 PASS, check 4-dim life-changing
...
def main():
    logger.info("paradigm 181 R-1 continuous-weighted long-only daily-rebal")
    df_close, df_ret = load_daily_returns()
    logger.info("daily close: %d days × %d syms", len(df_close), df_close.shape[1])

    # Compute z-scores
    z = compute_z_scores(df_close, RETURN_WINDOW_D, Z_WINDOW_D)
    logger.info("z-score panel shape: %s, NaN ratio: %.2f%%", z.shape, z.isna().mean().mean() * 100)
    ...
    logger.info("aligned panel: %d days × %d syms", len(weights), weights.shape[1])
    ...
    result = {
        "paradigm_id": 181,
        "paradigm_slug": "alt_per_sym_30d_return_z_continuous_weighted_long_only_position_size_daily_rebal",
        "phase": "R-1",
        "verdict": verdict,
    ...
    # Also dump portfolio time series
    ts_df = pd.DataFrame({
        "gross_ret": gross_ret,
        "net_ret": net_ret,

"""R-1 PoC — funding_velocity_cross_section_dispersion (P1, batch ad-hoc 2026-05-19).
...
Mechanism distinct from funding_dispersion R-5 seeded (cs z of funding LEVEL).
- statistic class: Δ (velocity) vs L (level) → DNA axis 2 mismatch
- expected DNA cross-check ≤ 4/6 with funding_dispersion ETC R-5

Protocol (paradigm-architect spec)
==================================
- _perm_utils.fee_aware_perm_test + bootstrap_ci
- 3-gate strict: sig_t_excess >= 2.0 AND ci_lower > 0 AND perm_p_two_sided <= 0.10
- Lesson #16 Concentration Gate per focus cell
- Lesson #19 Symmetric Negative Test (4-quadrant single batch)
- Life-changing 4-dim metrics per cell

4-quadrant cells (joint-trigger = cross-section + direction)
============================================================
A focus  : cs_z_Δf >  +Z_THR   × LONG   (high-Δ outlier → mean-reversal fade UP overshoot)
A mirror : cs_z_Δf >  +Z_THR   × SHORT
B mirror : cs_z_Δf <  -Z_THR   × LONG
B focus  : cs_z_Δf <  -Z_THR   × SHORT  (low-Δ outlier → mean-reversal fade DOWN overshoot)

Universe: 14 OHLCV-aligned syms (paradigm 69/73/96 standard)
...
log = logging.getLogger("p1_funding_velocity_cs_r1")
...
COOLDOWN_MIN = 8 * 60  # one cycle minimum

FEE_PER_TRADE = 0.0008  # 8bp round-trip

PARADIGM = "funding_velocity_cross_section_dispersion"
...
def build_panel() -> pd.DataFrame:
    """Build per-symbol funding cycle panel with Δf and forward returns."""
    frames = []
    per_sym_diag = {}
    for sym in SYMBOLS:
        ...
        if close.empty:
            log.warning("[%s] no ohlcv cache", sym)
            per_sym_diag[sym] = {"n_cycles": 0}
            continue

        f = fund.set_index("funding_time")["funding_rate"]
        delta_f = f.diff(periods=1)  # Δf = current - prev cycle
        ...
        df = pd.DataFrame({"delta_f": delta_f.values, **{c: rets[c] for c in HOLD_MINUTES}}, index=f.index)
        df["symbol"] = sym
        df = df.dropna(subset=["delta_f", FOCUS_HOLD_COL])
        frames.append(df)
        per_sym_diag[sym] = {
            "n_cycles": int(len(df)),
        ...
        log.info("[%s] %d funding cycles with Δf + fwd_ret features", sym, len(df))
    ...
    # Cross-section z-score per cycle timestamp
    log.info("Computing cross-section z-score of Δf per cycle (universe-wide median/std) ...")
    cs_z = []
    for ts, sub in panel.groupby(level=0):
        vals = sub["delta_f"].values
        if len(vals) < 3:
    ...
    panel = panel.copy()
    panel["cs_z_delta_f"] = cs_z
    panel = panel.dropna(subset=["cs_z_delta_f"])
...
def life_changing_metrics(obs_net: np.ndarray, hold_minutes: int, panel_span_days: float) -> dict:
    """4-dim life-changing gate: trades/yr / per_trade_edge / capital_util / annualized_sharpe."""
    n = len(obs_net)
    ...
    # capital_util: hold concurrency proxy = trades_per_year * hold_days / 365.25
    hold_days = hold_minutes / 1440.0
    capital_util = min(1.0, (trades_per_year * hold_days) / 365.25 * 365.25)  # actually: trades*hold_d/365.25
    capital_util = trades_per_year * hold_days / 365.25
    # annualized sharpe: per-trade sharpe * sqrt(trades/yr)
    ...
    return {
        "n_trades": int(n),
        "panel_span_days": float(panel_span_days),
...
def evaluate_quadrant(
    ...
    *,
    ...
    if n < 30:
        return {
            "cell": cell_label, "z_thr": z_thr, "hold_col": hold_col,
    ...
    result = {
        "cell": cell_label,
        "z_thr": z_thr, "hold_col": hold_col, "direction": direction,
...
def main() -> int:
    ...
    # A focus  : cs_z_Δf > +Z × LONG
    # A mirror : cs_z_Δf > +Z × SHORT
    # B mirror : cs_z_Δf < -Z × LONG
    # B focus  : cs_z_Δf < -Z × SHORT
    results = []
    for z_thr in Z_THRS:
        for hold_col in HOLD_MINUTES.keys():
            trig_high = apply_cooldown(panel[panel["cs_z_delta_f"] > z_thr].copy(), COOLDOWN_MIN)
            trig_low = apply_cooldown(panel[panel["cs_z_delta_f"] < -z_thr].copy(), COOLDOWN_MIN)
            log.info(
                "z=%.1f hold=%s | high=%d low=%d",
                z_thr, hold_col, len(trig_high), len(trig_low),
            )
    ...
    if not (a_pass_3g or b_pass_3g or am_pass_3g or bm_pass_3g):
        ...
        # both directions PASS at three-gate — degenerate (any-direction noise)
        verdict = "BROAD_FALSIFIED_DEGENERATE_BOTH_DIRECTIONS"
    elif any_focus_pass:
        # focus PASS three-gate, check Concentration Gate
        focus_cell = "A_focus" if a_pass_3g else "B_focus"
        ...
        # only mirror PASS — inverted mechanism, antipattern caution
    ...
    out = {
        ...
        "campaign": "batch_ad_hoc_R1_funding_family_distinct_P1",
        "hypothesis": "8h funding Δf cross-section z-score |cs_z_Δf|>2 → mean-reversal fade",
        "dna_distinct_from": [
            "funding_dispersion_R5_seeded_ETC (statistic L vs Δ)",
            "paradigm 96 sign flip (categorical vs continuous)",
            "paradigm 73 funding×OI joint (single axis vs joint)",
        ],
        "universe": SYMBOLS,
        "z_thresholds": Z_THRS,
        ...
        "focus_A_LONG": fa,
        "focus_A_mirror_SHORT": fa_m,
        "focus_B_mirror_LONG": fb_m,
        "focus_B_SHORT": fb,
        "verdict_components": {
            "A_focus_high_LONG_three_gate": a_pass_3g,
            "A_focus_concentration_gate": a_pass_cg,
            "A_focus_life_changing": a_pass_lc,
            "A_mirror_high_SHORT_three_gate": am_pass_3g,
            "B_mirror_low_LONG_three_gate": bm_pass_3g,
            "B_focus_low_SHORT_three_gate": b_pass_3g,
            "B_focus_concentration_gate": b_pass_cg,
            "B_focus_life_changing": b_pass_lc,
        },
        "verdict": verdict,
    }

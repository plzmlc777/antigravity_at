...
def simulate(cl, zv, za, live, r: Run, cooldown_min: int,
    ...
    for t in steps:
        ...
        for j in list(book):
            ...
            hit = False
            if r.stop_pct > 0 and seg.size:
                hit = bool((seg >= px0 * (1 + r.stop_pct / 100)).any() if sh
                           else (seg <= px0 * (1 - r.stop_pct / 100)).any())
    ...
    return trades, n_stop


def report(tag: str, trades, n_stop: int, r: Run) -> dict:
    if not trades:
        return {"tag": tag, "n": 0}
    ...
    return {"tag": tag, "n": len(net), "mean": float(net.mean()),
            ...
            "stops": n_stop, "compound": float(100 * (e[-1] - 1)),
            "mdd": float(mdd),
...
def main() -> int:
    ...
    rows, plc = [], {}
    for cd in cds:
        tr, ns = simulate(cl, zv, za, lv, r, cd)
        rows.append(report(f"냉각 {cd}분", tr, ns, r))
        log.info("냉각 %d분 — 거래 %d · %.1f분", cd, len(tr), (time.time() - t0) / 60)
        if cd > 0 and a.placebo_reps > 0:
            got = []
            for k in range(a.placebo_reps):
                g = np.random.default_rng(r.seed + 1000 * cd + k)
                t_, s_ = simulate(cl, zv, za, lv, r, cd, placebo_rng=g)
                got.append(report(f"위약 {cd}분 #{k}", t_, s_, r))
            plc[cd] = got

    print(f"\n■ 손절 후 재진입 금지 — 종목 {len(names)} · "
          f"{lo.date()}~{hi.date()} · 손절 {r.stop_pct:.0f}% · 슬롯 {r.slots}")
    print(f"  {'설정':>12}{'거래':>8}{'거래당%':>10}{'t':>7}{'승률':>7}"
          f"{'손절':>7}{'복리%':>10}{'최대낙폭%':>11}")
    for x in rows:
        if not x["n"]:
            continue
        print(f"  {x['tag']:>12}{x['n']:>8,}{x['mean']:>+10.4f}{x['t']:>+7.2f}"
              f"{x['win']:>6.0f}%{x['stops']:>7,}{x['compound']:>+10.2f}"
              f"{x['mdd']:>11.2f}")

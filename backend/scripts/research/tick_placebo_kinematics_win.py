...
log = logging.getLogger("kine_win")

WIN_H = 60          # 승률을 정의하는 지평(분)
WINDOW = 360        # 승률을 재는 창(분)
DELTA = 180         # 속도·가속을 재는 간격(분)
FWD = (30, 60, 120)
# ⚠ 굵은 구간(7칸)으로는 **꼬리가 안 보인다**. 슬롯 1 이 매번 고르는 자리가
#   z_vel 평균 −1.544 였는데 그 아래 구간이 표에 아예 없었다(사건이 적어
#   최소 표본에 걸림). 재보지도 않은 영역을 진입 규칙으로 박은 셈이다.
#   그래서 꼬리를 촘촘히 쪼갠다.
EDGES = (-5.0, -4.0, -3.0, -2.5, -2.0, -1.75, -1.5, -1.25, -1.0,
         -0.75, -0.5, -0.25, 0.0, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5,
         2.0, 2.5, 3.0)
LABELS = ("≤-5", "-5~-4", "-4~-3", "-3~-2.5", "-2.5~-2", "-2~-1.75",
          "-1.75~-1.5", "-1.5~-1.25", "-1.25~-1", "-1~-0.75", "-0.75~-0.5",
          "-0.5~-0.25", "-0.25~0", "0~0.25", "0.25~0.5", "0.5~0.75",
          "0.75~1", "1~1.25", "1.25~1.5", "1.5~2", "2~2.5", "2.5~3", "≥3")
...
def build(sym: str, cfg: Cfg):
    ...
    if cfg.hours:
        # ⚠ 최근 N시간만. 워밍업(지평60+창360+간격360=780분)과 선도 120분을
        #   빼면 24시간에서 쓸 수 있는 앵커는 540분치뿐이다 — 표본이 얇다.
        cut = (time.time() - cfg.hours * 3600) * 1000
        t = t[t.ts_ms >= cut]
...
def scan(S, cfg: Cfg, shift=None) -> dict:
    ...
    for k, v in acc_.items():
        ...
        # ⚠ 꼬리 칸은 원래 얇다. 200 이면 통째로 사라진다. 50 으로
        #   낮추되 그 이하는 여전히 버린다 — 5건짜리 칸으로 결론 내지 않는다.
        if len(r) < 50:
            continue
...
def main() -> int:
    ...
    path = Path(a.out) if a.out else OUT / "tick_placebo_kinematics_win.csv"

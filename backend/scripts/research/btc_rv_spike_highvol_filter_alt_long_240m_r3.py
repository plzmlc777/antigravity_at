...
def main():
    ...
    try:
        ...
        # ============================================================
        # G. SAMPLE BIAS — Q4-2025 vs Q1-2026 independent
        # ============================================================
        log.info("[G] sample bias — Q4-2025 vs Q1-2026 independent perm ...")
        ts_pd = pd.DatetimeIndex(ts_arr)
        q4_mask = (ts_pd >= pd.Timestamp("2025-10-01")) & (ts_pd < pd.Timestamp("2026-01-01"))
        q1_mask = (ts_pd >= pd.Timestamp("2026-01-01")) & (ts_pd < pd.Timestamp("2026-04-01"))
        q4_nets = nets[np.asarray(q4_mask)]
        q1_nets = nets[np.asarray(q1_mask)]
        ...
        q4_stat = quarter_stats(q4_nets, "2025Q4")
        q1_stat = quarter_stats(q1_nets, "2026Q1")
        out["sample_bias_quarters"] = {"q4_2025": q4_stat, "q1_2026": q1_stat}
        log.info("  Q4: %s", q4_stat)
        log.info("  Q1: %s", q1_stat)

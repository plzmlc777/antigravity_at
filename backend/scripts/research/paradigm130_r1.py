"""paradigm 130 R-1 PoC — alt_realized_corr_breakdown_eth_per_pair_directional_4h.

4-quadrant SNT (Lesson #19 mandatory):
  A_focus  = trigger ∩ direction>0 × LONG    (corr<p10 ∩ alt up → continuation)
  A_mirror = trigger ∩ direction>0 × SHORT   (corr<p10 ∩ alt up → fade)
  B_focus  = trigger ∩ direction<0 × SHORT   (corr<p10 ∩ alt down → continuation)
  B_mirror = trigger ∩ direction<0 × LONG    (corr<p10 ∩ alt down → reversion)
...
Lesson #52 candidate detection (3rd dogfood after paradigm 99/129):
  If both LONG quadrants gross > 0 AND both SHORT quadrants gross < 0 AND
  per-symbol Concentration Gate ci_pos count < 3 in any quadrant →
  flag as UNIVERSE_LONG_DRIFT_ARTIFACT

R-0 advisory: A_focus +31.73bp / B_focus +20.74bp stratified t<1.5 but
A_focus 2024Q1 +113bp / 2024Q4 +16bp / 2025Q3 -19bp shows progressive
decay → R-1 must confirm full-pool vs LONG-drift attribution.
"""
...
CORR_ROLLING_BARS_4H = 180
CORR_PERCENTILE = 10.0
FORWARD_HOLD_4H_BARS = 1
DEBOUNCE_HOURS = 8
FEE_PER_TRADE = 0.0016  # 16bp round-trip = 0.0016
...
def build_panel():
    ...
    eth = load_4h(BENCHMARK, engine)
    if eth.empty:
        log.error("ETH benchmark empty — halt")
        sys.exit(1)
    eth["log_ret_4h"] = np.log(eth["close"]).diff()
    ...
    for sym in COHORT:
        ...
        joined = df.join(eth[["log_ret_4h"]].rename(
            columns={"log_ret_4h": "eth_log_ret_4h"}), how="inner")
        if len(joined) < CORR_ROLLING_BARS_4H * 2:
            log.warning("%s skip insufficient after join", sym)
            continue
        ...
        # Per-pair p10 threshold
        rho_valid = joined["rho_30d"].dropna()
        if len(rho_valid) < CORR_ROLLING_BARS_4H:
            log.warning("%s skip insufficient rho samples", sym)
            continue
        p10 = float(np.percentile(rho_valid.values, CORR_PERCENTILE))
        joined["rho_p10_pair"] = p10
        joined["cond"] = joined["rho_30d"] <= p10
        joined["sym"] = sym
        panels[sym] = joined
    return panels
...
def build_candidate_pool(panel: dict) -> np.ndarray:
    """Unconditional fwd_ret pool for fee_aware_perm_test."""
    pool = []
    for sym, df in panel.items():
...
def detect_lesson_52_long_drift_artifact(quad_results: dict, conc_results: dict) -> dict:
    """Explicit Lesson #52 candidate detection (3rd dogfood after p99/p129)."""
    a_focus = quad_results.get("A_focus_rho_p10_pos_LONG_4h", {})
    a_mirror = quad_results.get("A_mirror_rho_p10_pos_SHORT_4h", {})
    b_focus = quad_results.get("B_focus_rho_p10_neg_SHORT_4h", {})
    b_mirror = quad_results.get("B_mirror_rho_p10_neg_LONG_4h", {})

    a_focus_gross = a_focus.get("gross_bp", 0)
    a_mirror_gross = a_mirror.get("gross_bp", 0)
    b_focus_gross = b_focus.get("gross_bp", 0)
    b_mirror_gross = b_mirror.get("gross_bp", 0)

    both_long_pos = (a_focus_gross > 0 and b_mirror_gross > 0)
    both_short_neg = (a_mirror_gross < 0 and b_focus_gross < 0)

    # Per-symbol Concentration ci_pos count for both LONG quadrants
    a_focus_ci_pos = conc_results.get("A_focus_LONG", {}).get("n_ci_pos", 0)
    b_mirror_ci_pos = conc_results.get("B_mirror_LONG", {}).get("n_ci_pos", 0)
    long_ci_count_low = (a_focus_ci_pos < 3 or b_mirror_ci_pos < 3)

    is_long_drift_artifact = bool(both_long_pos and both_short_neg and long_ci_count_low)
    return {
        "is_long_drift_artifact": is_long_drift_artifact,
        "both_long_pos": bool(both_long_pos),
        "both_short_neg": bool(both_short_neg),
        "long_ci_count_low": bool(long_ci_count_low),
        "a_focus_LONG_gross_bp": a_focus_gross,
        "b_mirror_LONG_gross_bp": b_mirror_gross,
        "a_mirror_SHORT_gross_bp": a_mirror_gross,
        "b_focus_SHORT_gross_bp": b_focus_gross,
        "a_focus_LONG_n_ci_pos": int(a_focus_ci_pos),
        "b_mirror_LONG_n_ci_pos": int(b_mirror_ci_pos),
        "dogfood_number": 3,
        "precedent": "paradigm 99 funding per-sym velocity, paradigm 129 parkinson range",
    }
...
def main():
    ...
    panel = build_panel()
    log.info("panel built: %d alts + benchmark %s", len(panel), BENCHMARK)
    ...
    quadrants = {
        "A_focus_rho_p10_pos_LONG_4h": (a_focus_net, "A_focus_LONG"),
        "A_mirror_rho_p10_pos_SHORT_4h": (a_mirror_net, "A_mirror_SHORT"),
        "B_focus_rho_p10_neg_SHORT_4h": (b_focus_net, "B_focus_SHORT"),
        "B_mirror_rho_p10_neg_LONG_4h": (b_mirror_net, "B_mirror_LONG"),
    }
    quad_results = {}
    ...
    # Lesson #52 candidate detection (3rd dogfood)
    log.info("--- Lesson #52 candidate detection (universe LONG drift artifact) ---")
    lesson_52 = detect_lesson_52_long_drift_artifact(quad_results, conc_results)
    log.info("Lesson #52 detection: %s", lesson_52)
    ...
    a_focus_neg = is_neg("A_focus_rho_p10_pos_LONG_4h")
    a_mirror_neg = is_neg("A_mirror_rho_p10_pos_SHORT_4h")
    b_focus_neg = is_neg("B_focus_rho_p10_neg_SHORT_4h")
    b_mirror_neg = is_neg("B_mirror_rho_p10_neg_LONG_4h")

    # Lesson #52 takes priority — if LONG drift artifact, verdict reflects that
    if lesson_52["is_long_drift_artifact"] and not pass_quadrants:
        verdict = "BROAD_FALSIFIED_FEE_FLOOR_LONG_DRIFT_ARTIFACT"
        sub_class = ("Lesson #52 3rd dogfood — universe LONG drift artifact "
                     "(A focus LONG + B mirror LONG both gross > 0, "
                     "A mirror SHORT + B focus SHORT both gross < 0, "
                     "ci_pos count < 3 in LONG quadrants confirms drift attribution)")
    elif not pass_quadrants:
    ...
    out = {
        ...
        "lesson_52_candidate_detection": lesson_52,
    }

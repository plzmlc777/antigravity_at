...
# 손절 물리 하한 — **2026-08-15 재측정으로 근거 갱신**(값은 유지).
#
# 커널은 진입한 바에서 SL/TP 를 검사하지 않으므로(미래참조 방지, 옳다),
# 그 바의 상승폭보다 좁은 손절은 **기록되지 않는다**. 얼마나 자주인가:
#
#   354상장 · 복구된 `ohlcv_daily` · 체결 바 (고가-시가)/시가
#     체결바          p50     p75     p90     평균
#     lag1(상장+1일)  8.1%   20.8%   38.2%   17.1%
#     lag2(상장+2일)  6.4%   14.1%   28.9%   12.1%   ← EXEC_LAG_BARS=2 기본
#
#     손절     lag1 무력화   lag2 무력화
#       2%       85.0%        79.9%
#       5%       62.7%        58.5%
#      10%       44.1%        35.6%
#      20%       26.6%      **16.9%**   ← 하한
...
#      50%        6.2%         3.7%
#
# 0.20 이면 lag2 기준 16.9% 다. 완전히 깨끗한 값은 없지만(0.50 도 3.7%),
# 여기 아래로는 급격히 나빠진다(0.10 → 35.6%).
#
# ⚠ **2026-08-15 정정** — 한때 "0.20 이 60% 무력화"라고 봤는데 그건
#   **상장일 부분봉**(상장 직후 급등 포함, p50 32.7%)을 진입 바로 착각한
#   것이었다. 실제 진입 바는 그 다음 날이다. 무력화율을 잴 때 **어느 바가
#   체결 바인지** 먼저 확정하라.
#
# ⚠ 이 하한은 **일봉 harness 한정**이다. 1h 로 내리면 체결 바 상승폭이
#   훨씬 작아 더 좁은 손절도 측정 가능해진다 (`lifecycle_1h_backtest`).
SL_FLOOR = 0.20

# 손절값별 무력화율 (lag2 기준, 354상장 실측). 표에 함께 찍어 눈에 보이게 한다 —
# 하한만 두면 통과한 칸이 얼마나 오염됐는지 알 수 없다.
SL_NULLIFY_PCT = {0.02: 79.9, 0.05: 58.5, 0.10: 35.6, 0.20: 16.9,
                  0.30: 9.6, 0.50: 3.7, 0.70: 1.4, 1.00: 0.6}
...
def main() -> int:
    ...
    print("-" * 92)
    # 손절 무력화율 — 하한을 통과한 칸도 오염돼 있을 수 있다. 숫자를 보여준다.
    _sls = sorted({float(c[names.index("sl")]) for c in combos}
                  if "sl" in names else set())
    if _sls:
        print("  **손절 무력화율** (체결 바 상승폭 > 손절폭 = 그 손절은 기록 안 됨)")
        _cells = []
        for _sl in _sls:
            _near = min(SL_NULLIFY_PCT, key=lambda k: abs(k - _sl))
            _pct = SL_NULLIFY_PCT[_near]
            _flag = " ⚠" if _pct >= 20 else ""
            _cells.append(f"{_sl:.0%}→{_pct:.1f}%{_flag}")
        print("     " + " · ".join(_cells))
        print("     (354상장 실측 · lag2 기준. ⚠ = 20% 이상이 기록되지 않는다)")
        print("     좁은 손절을 제대로 재려면 1h harness: "
              "scripts.research.lifecycle_1h_backtest")

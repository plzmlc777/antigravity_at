#!/usr/bin/env python3
"""Phase R-3 robustness for range_expansion (n=200 perm test).

Permutation: shuffle high/low pair (preserve open/close) — destroys range
timing but preserves distribution. If signal is timing-dependent, shuffled
...
from scripts.poc_range_expansion import (  # noqa: E402
    PARADIGM, OUT_DIR, load_ohlc_5m, simulate,
)

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("poc_range_expansion_r3")
...
def main():
    ...
    p.add_argument("--symbols", nargs="+", default=["SOLUSDT", "DOGEUSDT"])
    p.add_argument("--range-zwin", type=int, default=288)
    ...
    rule_params = dict(
        range_zwin=args.range_zwin, entry_z=args.entry_z,

#!/usr/bin/env python3
"""Phase R-3 robustness for bid_ask_concentration_regime (n=200 perm test)."""
...
from scripts.poc_bid_ask_concentration_regime import (  # noqa: E402
    PARADIGM, OUT_DIR, load_close_1d, load_concentration_1d, simulate,
)

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("poc_bid_ask_concentration_regime_r3")


def permutation_test(close, conc, n_iter, real_alpha, **rule_params):
    rng = np.random.default_rng(seed=42)
    base = conc.values.astype(float)
    random_alphas = []
    for it in range(n_iter):
        shuf = rng.permutation(base)
        shuf_series = pd.Series(shuf, index=conc.index)
...
def main() -> int:
    ...
    p.add_argument("--symbols", nargs="+",
                   default=["BTCUSDT", "ETHUSDT", "DOGEUSDT", "SOLUSDT"])
    ...
    for sym in args.symbols:
        try:
            ...
            real = simulate(close, conc, train_frac=0.5, **rule_params)
            ...
            perm = permutation_test(close, conc, args.n_iter_perm, real_alpha,
                                    **rule_params)

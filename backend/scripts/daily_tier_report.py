...
**실전 리그** = 승강제가 있는 조직(3군 발굴 → 2군 검증 → 1군 실거래).
실계좌가 걸린 유일한 리그다. 초단타·단타·미장 리그와 병렬이며 그쪽은
승격·강등이 없다.

명명 이력 — 2026-08-14 "패러다임 리그" → 2026-08-23 **"실전 리그"**
(대표님 지시). 실자금이 걸렸다는 사실을 이름이 먼저 말하게 한다.
...
def build() -> str:
    ...
    head = [f"📊 <b>실전 리그 일일 리포트</b> {now:%Y-%m-%d (%a)}", ""]
    tail = ["", "<i>3군 디스패치 정지(2026-08-13) 대체 리포트</i>"]
    return "\n".join(head + tier1() + tier2() + tail)


def main() -> int:
    ap = argparse.ArgumentParser(description="실전 리그 일일 리포트")

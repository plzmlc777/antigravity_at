#!/usr/bin/env python3
"""Phase R-1 PoC: Premium-OI joint filter paradigm.

Signal: premium z-score fires (|prem_z| > entry_z) AND OI z-score direction
agrees with premium direction. Trade only when both confirm. Compare to
premium-only signal (= premium_index_zscore baseline).

§3-G filter mechanism (joint_3signal_ensemble 2-signal version,
component-seeds 결합). joint_3signal_ensemble was POSITIVE (DOGE 9.6σ but
all candidates already seeded). Expected: 2-signal version weaker.
"""
...
log = logging.getLogger("poc_premium_oi_joint_filter")

PARADIGM = "premium_oi_joint_filter"
...
def simulate(close: pd.Series, premium: pd.Series, oi: pd.Series, *,
             zwin: int, prem_entry_z: float, oi_min_z: float, hold_days: int,
             sl_pct: float, fee_rate: float, capital: float, train_frac: float,
             ) -> dict:
    df = pd.concat({"close": close, "premium": premium, "oi": oi},
    ...
    df = df.dropna(subset=["prem_z", "oi_z"])
    ...
    for i in range(1, len(test)):
        ...
        if not in_pos:
            row = test.iloc[i]
            pz = float(row["prem_z"]); oz = float(row["oi_z"])
            if math.isnan(pz) or math.isnan(oz):
                equity_curve.append((ts, equity)); continue
            # premium signal fires + OI direction confirms
            if abs(pz) > prem_entry_z and abs(oz) > oi_min_z and pz * oz > 0:
                # follow premium direction (premium_index_zscore baseline)
...
def main() -> int:
    ...
    p.add_argument("--prem-entry-z", type=float, default=2.0)
    p.add_argument("--oi-min-z", type=float, default=0.5)
    ...
    tag = args.tag or f"poc_zw{args.zwin}_pez{args.prem_entry_z}_oiz{args.oi_min_z}_h{args.hold_days}"
    ...
    for sym in args.symbols:
        try:
            ...
            sim = simulate(close, premium, oi,
                           zwin=args.zwin, prem_entry_z=args.prem_entry_z,
                           oi_min_z=args.oi_min_z, hold_days=args.hold_days,

#!/usr/bin/env python3
"""Phase R-1 PoC: Premium intraday range z-score paradigm.

Hypothesis: Extreme premium range (high-low) = positioning uncertainty
event. Direction inferred from prior price return direction.
  - 'fade': extreme range + prior up → short (uncertainty resolves down)
  - 'follow': extreme range + prior up → long (vol = momentum)

Distinct from #1 premium_volatility_regime (graveyard 2026-05-06):
  - #1 used (range - mean) / std normalization, direction from close premium sign
  - This: ratio range/median(range), then z-score of ratio. direction from prior price.
  - Different normalization (robust median) + different direction logic.

§3-G volume_absorption family (range-based event paradigms).
"""
...
log = logging.getLogger("poc_premium_intraday_range_zscore")

PARADIGM = "premium_intraday_range_zscore"
...
def load_premium_ohlc(symbol: str) -> pd.DataFrame:
    ...
    df = joblib.load(p)
    out = df[["high", "low"]].astype(float).copy()
...
def simulate(close: pd.Series, prem_ohlc: pd.DataFrame, *,
    ...
    df["range_med"] = df["prem_range"].rolling(zwin).median()
    df["range_ratio"] = df["prem_range"] / df["range_med"]
    df["range_ratio_z"] = (
        (df["range_ratio"] - df["range_ratio"].rolling(zwin).mean())
        / df["range_ratio"].rolling(zwin).std()
    )
    df["prior_ret"] = df["close"].pct_change(ret_lookback)
    df = df.dropna(subset=["range_ratio_z", "prior_ret"])
    ...
    for i in range(1, len(test)):
        ...
        if not in_pos:
            row = test.iloc[i]
            rz = float(row["range_ratio_z"]); pr = float(row["prior_ret"])
            if math.isnan(rz) or math.isnan(pr):
                equity_curve.append((ts, equity)); continue
            if rz > entry_z and abs(pr) > 0:
                if mode == "fade":
...
def main() -> int:
    ...
    tag = args.tag or (
        f"poc_zw{args.zwin}_rl{args.ret_lookback}_ez{args.entry_z}_h{args.hold_days}_{args.mode}"
    )

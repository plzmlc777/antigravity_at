#!/usr/bin/env python3
"""Phase R-1 PoC: Multi-zwin ensemble premium paradigm.

Signal: premium 15d, 30d, 60d z-scores. Entry when all three signs agree
AND |sum|>z_sum_thresh. Direction = sign of sum.

§3-G: premium_index_zscore (seeded, zwin=30, 9σ) timeframe ensemble.
R-1: 발화 빈도 낮을 듯, 즉시 측정.
"""
...
log = logging.getLogger("poc_multi_zwin_ensemble_premium")

PARADIGM = "multi_zwin_ensemble_premium"
...
def simulate(close: pd.Series, premium: pd.Series, *,
             zwins: tuple[int, ...], z_sum_thresh: float, hold_days: int,
    ...
    for w in zwins:
        col = f"z{w}"
        df[col] = (df["premium"] - df["premium"].rolling(w).mean()) / df["premium"].rolling(w).std()
        z_cols.append(col)
    df = df.dropna(subset=z_cols)
    df["z_sum"] = df[z_cols].sum(axis=1)
    df["sign_agree"] = df[z_cols].apply(lambda row: int(np.all(row > 0)) - int(np.all(row < 0)), axis=1)
    ...
    for i in range(1, len(test)):
        ...
        if not in_pos:
            row = test.iloc[i]
            zs = float(row["z_sum"]); sa = int(row["sign_agree"])
            if math.isnan(zs):
                equity_curve.append((ts, equity)); continue
            if sa != 0 and abs(zs) > z_sum_thresh:
                side = 1 if zs > 0 else -1
                if side > 0:
                    n_long += 1
                else:
                    n_short += 1
...
def main() -> int:
    ...
    tag = args.tag or f"poc_zwins{'_'.join(map(str, args.zwins))}_zsum{args.z_sum_thresh}_h{args.hold_days}"
    ...
    for sym in args.symbols:
        try:
            ...
            sim = simulate(close, premium,
                           zwins=tuple(args.zwins), z_sum_thresh=args.z_sum_thresh,

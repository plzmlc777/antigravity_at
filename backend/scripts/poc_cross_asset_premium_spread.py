#!/usr/bin/env python3
"""Phase R-1 PoC: Cross-asset premium spread paradigm.

Hypothesis: (alt_premium_z) - (BTC_premium_z) at daily granularity
captures alt-specific over/under-positioning relative to BTC. Extreme
spread → relative basis mean-reverts.

Distinct from seeded `premium_index_zscore`:
  - Premium INDEX z = absolute deviation per symbol.
  - This = relative deviation (alt minus BTC), neutralizing systemic
    premium drift (when BTC funding rises, all premiums rise; spread
    isolates alt-specific demand pressure).

Distinct from seeded `funding_dispersion`:
  - Funding dispersion = cross-section z over 14 syms at 8h (clamped/settled).
  - This = bilateral alt-vs-BTC spread at 1d (raw premium).

§3 risks:
  - §3-G family: premium domain saturated. Mitigated: spread is a
    different statistical construct (relative vs absolute) — analogous
    to pairs trading vs single-symbol z.
  - cross_symbol_correlation_regime (graveyard) — different dimension.
  - §3-A rare-event: entry_z ≥ 1.5 sweep.

Entry rule:
  1. Compute prem_z per symbol (zwin=30d) and BTC prem_z.
  2. spread_z = alt_prem_z - btc_prem_z, then 30d z-score of the spread.
  3. |spread_zz| > entry_z → trade.
     - mode 'fade': alt over BTC → SHORT alt.
     - mode 'follow': alt over BTC → LONG alt.
"""
...
log = logging.getLogger("poc_cross_asset_premium_spread")

PARADIGM = "cross_asset_premium_spread"
...
def simulate(close: pd.Series, alt_prem: pd.Series, btc_prem: pd.Series, *,
    ...
    df["alt_z"] = (df["alt_prem"] - df["alt_prem"].rolling(zwin).mean()) / df["alt_prem"].rolling(zwin).std()
    df["btc_z"] = (df["btc_prem"] - df["btc_prem"].rolling(zwin).mean()) / df["btc_prem"].rolling(zwin).std()
    df["spread"] = df["alt_z"] - df["btc_z"]
    df["spread_zz"] = (df["spread"] - df["spread"].rolling(zwin).mean()) / df["spread"].rolling(zwin).std()
    df = df.dropna(subset=["spread_zz"])
...
def main() -> int:
    ...
    btc_prem = load_premium_close_1d("BTCUSDT")
    log.info("BTC premium: %d days", len(btc_prem))

    rows = []
    for sym in args.symbols:
        try:
            close = load_close_1d(sym)
            alt_prem = load_premium_close_1d(sym)
            log.info("%s close: %d days, premium: %d days", sym, len(close), len(alt_prem))
            sim = simulate(
                close, alt_prem, btc_prem,

#!/usr/bin/env python3
"""Phase R-3 robustness for monthly_premium_seasonality (n=200 perm test).

Permutation: shuffle premium series (preserve calendar mask + price).
...
from scripts.poc_monthly_premium_seasonality import (  # noqa: E402
    PARADIGM, OUT_DIR, load_close_1d, load_premium_close_1d, simulate,
)

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("poc_monthly_premium_seasonality_r3")
...
def main() -> int:
    ...
    p.add_argument("--symbols", nargs="+", default=["SOLUSDT", "DOGEUSDT", "UNIUSDT"])
    ...
    p.add_argument("--mode", default="follow_eom")
    p.add_argument("--n-iter-perm", type=int, default=200)
    args = p.parse_args()

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    rule_params = dict(zwin=args.zwin, entry_z=args.entry_z, n_days=args.n_days,
    ...
    for sym in args.symbols:
        try:
            ...
            perm = permutation_test(close, premium, args.n_iter_perm,
            ...
            tag = f"r3_{args.mode}_{sym}_z{args.entry_z}_nd{args.n_days}_h{args.hold_days}"

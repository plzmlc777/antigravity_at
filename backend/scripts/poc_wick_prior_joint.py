#!/usr/bin/env python3
"""Phase R-1 PoC: Wick × Prior-Ret JOINT TRANSFORM (Q3 #6).

Q3 #2 wick_reversal POSITIVE 3σ used BINARY AND gates (wick_thresh AND
prior_move_pct). Q3 #4 wick_reversal_multibar elevated SOL to 4.49σ but
1/4 multi-symbol consistency = §3-C single-symbol-fit.

This paradigm: replace BINARY AND with CONTINUOUS multiplicative composite.
  composite = (lower_wick_frac - upper_wick_frac) * (-prior_ret_pct)
  composite_z = rolling-N z-score of composite
  entry: composite_z > entry_z (extreme aligned reversal magnitude)
         direction: sign(lower_wick_frac - upper_wick_frac)
           > 0 → LONG (lower wick dominant)
           < 0 → SHORT (upper wick dominant)

Mathematical structure:
  - composite > 0 when (wick_imbalance > 0 AND prior_ret < 0) OR
                       (wick_imbalance < 0 AND prior_ret > 0) — aligned reversal
  - composite < 0 when wick + ret in same direction (continuation)
  - magnitude depends on BOTH inputs proportionally (not binary)

Distinct from prior 53 paradigms:
  - wick_reversal Q3 #2: binary AND gates, threshold-fixed
  - wick_reversal_multibar Q3 #4: multi-bar avg, binary gates
  - This: continuous z-score of multiplicative composite — different mechanism

§3-H avoidance: this is NOT a filter on top of a signal. The composite IS
the signal — magnitude integrates wick and prior_ret as a unified metric.

§3-K compliance: SHAPE (wick) provides direction, magnitude carries
combined SHAPE × MAGNITUDE info.

Anti-pattern checks:
  - rare-event §3-A: R-1 sweep entry_z ∈ {1.5, 2.0, 2.5, 3.0}
  - in-sample §3-F: rolling z real-time
  - family-extension §3-G: wick variant — risk acknowledged
  - single-symbol-fit §3-C: must check multi-sym in R-2/R-3
  - magnitude-only §3-K: this uses SHAPE+MAGNITUDE, not magnitude only

Usage:
  python -m scripts.poc_wick_prior_joint --symbols SOLUSDT
"""
...
log = logging.getLogger("poc_wick_prior_joint")

PARADIGM = "wick_prior_joint"
...
def simulate(df: pd.DataFrame, *, zwin: int, entry_z: float,
             prior_lookback: int,
    ...
    df["lwf"] = (body_bot - df["low"]) / rng
    df["uwf"] = (df["high"] - body_top) / rng
    df["wick_imbalance"] = df["lwf"] - df["uwf"]
    df["prior_ret"] = df["close"].pct_change(prior_lookback)
    df["composite"] = df["wick_imbalance"] * (-df["prior_ret"])
    df["comp_z"] = (df["composite"] - df["composite"].rolling(zwin).mean()) / df["composite"].rolling(zwin).std()
    df = df.dropna(subset=["comp_z", "wick_imbalance"])
    ...
    for i in range(1, len(test)):
        ...
        if not in_pos:
            row = test.iloc[i]
            cz = float(row["comp_z"])
            wi = float(row["wick_imbalance"])
            if math.isnan(cz) or math.isnan(wi):
...
def main() -> int:
    ...
    tag = args.tag or (
        f"poc_zw{args.zwin}_ez{args.entry_z}_pl{args.prior_lookback}"
        f"_h{args.hold_bars}"
    )
    ...
    for sym in args.symbols:
        try:
            ...
            sim = simulate(
                df, zwin=args.zwin, entry_z=args.entry_z,

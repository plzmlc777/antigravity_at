#!/usr/bin/env python3
"""Phase R-1 PoC: Weekday DoW combined (Thu+Fri+Sat cluster) paradigm.

Hypothesis: weekend_drift_premium graveyard found pre-weekend cluster
(Thu/Fri/Sat) is strong, single-day Friday filter was 1.9-3.9σ borderline.
Combine 3-day cluster (DoW ∈ {3,4,5} = Thu, Fri, Sat) for richer entry window.

Signal: daily premium z-score (zwin=30) + DoW filter ∈ allowed_dows.
Direction: follow premium z (premium_index_zscore baseline).

§3-F calendar bias + §3-G premium domain saturation.
Per queue rule: "weekend_drift R-2 alpha 10/10 perm σ 약 → 통합도 비슷할 듯, R-3 perm 즉시"
"""
...
log = logging.getLogger("poc_weekday_DoW_combined")

PARADIGM = "weekday_DoW_combined"
...
def simulate(close: pd.Series, premium: pd.Series, *,
    ...
    for i in range(1, len(test)):
        ...
        if not in_pos:
            row = test.iloc[i]
            pz = float(row["prem_z"]); dow = int(row["dow"])
            if math.isnan(pz):
                equity_curve.append((ts, equity)); continue
            if dow in allowed_dows and abs(pz) > entry_z:
...
def main() -> int:
    ...
    p.add_argument("--allowed-dows", nargs="+", type=int, default=[3, 4, 5])
    p.add_argument("--tag", default=None)
    args = p.parse_args()

    tag = args.tag or f"poc_zw{args.zwin}_ez{args.entry_z}_h{args.hold_days}_dows{'_'.join(map(str, args.allowed_dows))}"
    ...
    for sym in args.symbols:
        try:
            ...
            sim = simulate(close, premium,
                           ...
                           allowed_dows=tuple(args.allowed_dows))
            sim["symbol"] = sym
            rows.append(sim)

#!/usr/bin/env python3
"""Phase R-3 robustness for cross_asset_premium_spread (n=200 perm test).

Permutation strategy: shuffle alt premium series (preserve BTC premium and
price intact). Tests whether timing of (alt_z - BTC_z) extremes truly
predicts alt mean-reversion or is just noise that happens to coincide with
alt corrections.
...
from scripts.poc_cross_asset_premium_spread import (  # noqa: E402
    PARADIGM, OUT_DIR, load_close_1d, load_premium_close_1d, simulate,
)

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("poc_cross_asset_premium_spread_r3")


def permutation_test(close, alt_prem, btc_prem, n_iter, real_alpha, **rule_params):
    rng = np.random.default_rng(seed=42)
    base = alt_prem.values.astype(float)
    random_alphas = []
    for it in range(n_iter):
        shuf = rng.permutation(base)
        shuf_series = pd.Series(shuf, index=alt_prem.index)
        sim = simulate(close, shuf_series, btc_prem, train_frac=0.5, **rule_params)
        if sim.get("error"):
...
def main() -> int:
    ...
    p.add_argument("--symbols", nargs="+",
                   default=["ETCUSDT", "LINKUSDT", "AVAXUSDT", "UNIUSDT"])
    ...
    for sym in args.symbols:
        try:
            ...
            real = simulate(close, alt_prem, btc_prem, train_frac=0.5, **rule_params)
            ...
            perm = permutation_test(close, alt_prem, btc_prem, args.n_iter_perm,

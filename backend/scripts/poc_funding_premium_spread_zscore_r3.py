#!/usr/bin/env python3
"""Phase R-3 robustness for funding_premium_spread_zscore (n=200 perm test).

Permutation: shuffle premium series only (preserve funding+price). Tests
whether timing of spread extremes (joint funding-premium divergence) truly
...
from scripts.poc_funding_premium_spread_zscore import (  # noqa: E402
    PARADIGM, OUT_DIR, load_close_1d, load_funding_1d, load_premium_close_1d, simulate,
)

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("poc_funding_premium_spread_zscore_r3")


def permutation_test(close, funding, premium, n_iter, real_alpha, **rule_params):
    ...
    for it in range(n_iter):
        ...
        sim = simulate(close, funding, shuf_series, train_frac=0.5, **rule_params)
        if sim.get("error"):
...
def main() -> int:
    ...
    p.add_argument("--symbols", nargs="+", default=["SOLUSDT", "ETCUSDT"])
    ...
    for sym in args.symbols:
        try:
            ...
            real = simulate(close, funding, premium, train_frac=0.5, **rule_params)
            ...
            perm = permutation_test(close, funding, premium, args.n_iter_perm,

#!/usr/bin/env python3
"""Phase R-3 robustness for funding_oi_phase_lag (n=200 perm test).

Permutation strategy: shuffle funding rate series only (preserve OI and
price). This randomizes the temporal phase indicator while leaving the
OI direction signal intact. If real_alpha exceeds shuffled significantly,
the lead-lag phase filter genuinely adds information beyond raw OI z.
...
from scripts.poc_funding_oi_phase_lag import (  # noqa: E402
    PARADIGM, OUT_DIR, load_close_1d, load_funding_1d, load_oi_1d, simulate,
)

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("poc_funding_oi_phase_lag_r3")


def permutation_test(close, funding, oi, n_iter, real_alpha, **rule_params):
    rng = np.random.default_rng(seed=42)
    base = funding.values.astype(float)
    random_alphas = []
    for it in range(n_iter):
        shuf = rng.permutation(base)
        shuf_series = pd.Series(shuf, index=funding.index)
        sim = simulate(close, shuf_series, oi, train_frac=0.5, **rule_params)
        if sim.get("error"):
...
def main() -> int:
    ...
    p.add_argument("--symbols", nargs="+",
                   default=["SOLUSDT", "UNIUSDT", "LINKUSDT", "AVAXUSDT"])
    ...
    for sym in args.symbols:
        try:
            ...
            oi = load_oi_1d(sym)
            real = simulate(close, funding, oi, train_frac=0.5, **rule_params)
            ...
            perm = permutation_test(close, funding, oi, args.n_iter_perm,
                                    real_alpha, **rule_params)
            ...
            tag = f"r3_{args.mode}_{sym}_z{args.entry_z}_pt{args.phase_thresh}_h{args.hold_days}"

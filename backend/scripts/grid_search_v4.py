"""Grid search v4 — bookDepth (B) source added.

Tests if order book imbalance (instant liquidity) provides decisive signal
that positioning-based sources (S/T/O/P) don't capture.

Data limitation: bookDepth backfill is 365d (vs 800d for other sources),
so OOS test period is shorter. Train period uses ohlcv only when bd is NaN.
"""
...
# Combos with B (bookDepth) as new dimension. Anchor: previous winners
SOURCE_COMBOS = [
    ("B",),                  # bookDepth only
    ("S", "B"),
    ("T", "B"),
...
def load_data(sym):
    ...
    bd_path = ROOT / "runs" / "book_depth" / f"{sym}_bookdepth.joblib"
    bd = joblib.load(bd_path) if bd_path.exists() else None
    return df_eval, signals, metrics, bd


def attach_combo(feat_pat, df_eval, metrics, bd_daily, combo, eval_freq_min=1440):
    out = feat_pat
    ...
    for code in combo:
        if code == "S":
            ...
            if bd_daily is None or bd_daily.empty: continue
            src = BinanceBookDepthSource(bd_daily=bd_daily)
            ctx = SourceContext(symbol="", eval_freq_minutes=eval_freq_min, ohlcv_eval=proxy)
            out = out.join(src.build_features(ctx), how="left")
    return out
...
def evaluate(sym, df_eval, signals, metrics, bd, combo, fwd, policy, threshold):
    ...
    feat = attach_combo(feat_pat, df_eval, metrics, bd, combo)
...
def main():
    print("Grid search v4 — bookDepth (B) source")
    n_total = (len(SYMBOLS) * len(SOURCE_COMBOS) * len(FORWARD_BARS)
               * len(POLICIES) * len(THRESHOLDS))
    print(f"  {n_total} conditions to test")

    results = []
    for sym in SYMBOLS:
        try:
            df_eval, signals, metrics, bd = load_data(sym)
            if signals is None or bd is None or bd.empty:
                print(f"[{sym}] SKIP — missing signals or bookDepth"); continue
        ...
        for combo, fwd, policy, thr in itertools.product(SOURCE_COMBOS, FORWARD_BARS, POLICIES, THRESHOLDS):
            try:
                r = evaluate(sym, df_eval, signals, metrics, bd, combo, fwd, policy, thr)
            except Exception:
                continue
    ...
    if not results:
        print("No results — likely backfill incomplete")
        return 1
    ...
    if len(sg) > 0:
        print(sg[["symbol", "combo", "fwd", "policy", "threshold", "sign", "binom_p", "alpha_pts", "alpha_vf_pts", "trades"]].to_string(index=False))
    ...
    print(show[["symbol", "combo", "fwd", "policy", "threshold", "sign", "binom_p", "alpha_pts", "alpha_vf_pts", "trades", "trades_vf"]].to_string(index=False))

    print("\n>>> Best per symbol (by sign, then alpha_vf):")
    for sym in SYMBOLS:
        sub = df[df["symbol"] == sym]
        if sub.empty: continue
        best = sub.sort_values(["sign", "alpha_vf_pts"], ascending=[False, False]).iloc[0]
        gate_ok = best["sign"] >= 0.60 and best["binom_p"] < 0.01
        print(f"  {sym:>10}: combo={best['combo']:14s} fwd={best['fwd']} pol={best['policy']:11s} "
              f"thr={best['threshold']:.3f} sign={best['sign']*100:.1f}% p={best['binom_p']:.4f} "
              f"alpha_vf={best['alpha_vf_pts']:+7.2f}pts  ({'PASS' if gate_ok else 'fail'})")

    out_path = ROOT / "runs" / "binance_grid_search_v4.csv"

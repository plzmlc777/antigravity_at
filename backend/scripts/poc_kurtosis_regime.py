#!/usr/bin/env python3
"""Phase R-1 PoC: Kurtosis Regime paradigm (4th moment).

Hypothesis: when rolling 60-bar excess kurtosis of 5m returns reaches extreme
percentile (top 10%), the distribution has fat tails — recent extreme outlier
event(s) dominating. This indicates volatility regime shift. Direction comes
from recent N-bar return sign:
  - DEFAULT (continuation): high kurt + recent up → LONG; high kurt + recent down → SHORT
  - REVERSAL (--reverse-sign): high kurt + recent up → SHORT; high kurt + recent down → LONG

Distinct from all 10 graveyard paradigms:
  - skewness_regime (3rd moment) gave perm_p 0.060 (best of session)
  - This paradigm tests "higher moment = closer to signal" hypothesis
  - kurtosis is unsigned (always positive in excess form measured here as kurt),
    so direction comes from a separate momentum signal — kurt acts as REGIME GATE

Pipeline:
  1. Load 1m → resample to 5m bars.
  2. Compute rolling KURT_WINDOW excess kurtosis of 5m log returns.
  3. Compute REGIME_WINDOW percentile rank of kurtosis.
  4. Compute DIR_LOOKBACK-bar return sign for direction.
  5. Entry when kurt_pctl > KURT_HIGH_PCTL:
     - dir_sign + default → LONG; dir_sign + reverse → SHORT
     - dir_sign − default → SHORT; dir_sign − reverse → LONG
  6. Exit at HOLD bars or SL.

Usage:
  python -m scripts.poc_kurtosis_regime --symbols SOLUSDT
"""
...
log = logging.getLogger("poc_kurtosis_regime")

PARADIGM = "kurtosis_regime"
...
def simulate_kurtosis_regime(df: pd.DataFrame, *, kurt_window: int, regime_window: int,
                              kurt_high_pctl: float, dir_lookback: int,
                              hold_bars: int, sl_pct: float, fee_rate: float,
                              capital: float, train_frac: float,
                              reverse_sign: bool = False) -> dict:
    df = df.copy()
    df["ret"] = np.log(df["close"] / df["close"].shift(1))
    df["kurt"] = df["ret"].rolling(kurt_window).kurt()
    df["kurt_pctl"] = df["kurt"].rolling(regime_window).rank(pct=True)
    df["dir_ret"] = df["close"].pct_change(dir_lookback).shift(1)
    df = df.dropna(subset=["kurt", "kurt_pctl", "dir_ret"])
    ...
    for i in range(1, len(test)):
        ...
        if not in_pos:
            row = test.iloc[i]
            pctl = float(row["kurt_pctl"])
            dir_ret = float(row["dir_ret"])
            if (not math.isnan(pctl) and pctl > kurt_high_pctl
                    and not math.isnan(dir_ret) and dir_ret != 0):
                if dir_ret > 0:
                    side = (-1 if reverse_sign else 1)  # default: LONG continuation
                else:
                    side = (1 if reverse_sign else -1)  # default: SHORT continuation
                in_pos = True
...
def main() -> int:
    ...
    p.add_argument("--kurt-window", type=int, default=60)
    p.add_argument("--regime-window", type=int, default=8640)
    p.add_argument("--kurt-high-pctl", type=float, default=0.90)
    ...
    p.add_argument("--reverse-sign", action="store_true",
                   help="Reversal hypothesis (high kurt + up trend → SHORT)")
    p.add_argument("--tag", default=None)
    args = p.parse_args()

    direction = "rev" if args.reverse_sign else "cont"
    tag = args.tag or (
        f"poc_{direction}_kw{args.kurt_window}_p{args.kurt_high_pctl}"
    ...
    for sym in args.symbols:
        try:
            ...
            sim = simulate_kurtosis_regime(
                df, kurt_window=args.kurt_window, regime_window=args.regime_window,
                kurt_high_pctl=args.kurt_high_pctl, dir_lookback=args.dir_lookback,

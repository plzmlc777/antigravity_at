#!/usr/bin/env python3
"""Phase R-1 PoC: Monthly premium seasonality paradigm.

Hypothesis: Institutional rebalancing flows concentrate at month-end (last
N days). Premium z-score at month-end is followed by month-start
normalization → fade end-of-month extreme.

Modes:
  - 'fade_eom': last N days of month, |prem_z|>entry_z → fade direction.
  - 'follow_eom': last N days of month, |prem_z|>entry_z → follow direction.
  - 'fade_bom': first N days of month, |prem_z|>entry_z → fade.

§3 risks: §3-F calendar bias (time_of_day_seasonality graveyard family).
Sparse: 12 events/year × 14 syms = 168 samples baseline.
"""
...
log = logging.getLogger("poc_monthly_premium_seasonality")

PARADIGM = "monthly_premium_seasonality"
...
def is_eom(idx: pd.DatetimeIndex, n_days: int) -> np.ndarray:
    """Mark last n_days of each month as True."""
    days_in_month = idx.days_in_month
    return idx.day > (days_in_month - n_days)


def is_bom(idx: pd.DatetimeIndex, n_days: int) -> np.ndarray:
    return idx.day <= n_days


def simulate(close: pd.Series, premium: pd.Series, *,
             zwin: int, entry_z: float, n_days: int,
             hold_days: int, sl_pct: float, fee_rate: float, capital: float,
             train_frac: float, mode: str = "fade_eom",
             ) -> dict:
    ...
    if mode in ("fade_eom", "follow_eom"):
        in_window = is_eom(test.index, n_days)
    else:
        in_window = is_bom(test.index, n_days)
    test["in_window"] = in_window
    ...
    for i in range(1, len(test)):
        ...
        if not in_pos:
            row = test.iloc[i]
            pz = float(row["prem_z"])
            if math.isnan(pz) or not bool(row["in_window"]):
                equity_curve.append((ts, equity)); continue
            if abs(pz) > entry_z:
                if mode in ("fade_eom", "fade_bom"):
...
def main() -> int:
    ...
    p.add_argument("--n-days", type=int, default=4)
    p.add_argument("--hold-days", type=int, default=3)
    p.add_argument("--sl-pct", type=float, default=0.05)
    p.add_argument("--fee-rate", type=float, default=0.0004)
    p.add_argument("--capital", type=float, default=1_000_000.0)
    p.add_argument("--train-frac", type=float, default=0.5)
    p.add_argument("--mode", choices=["fade_eom", "follow_eom", "fade_bom"],
                   default="fade_eom")
    p.add_argument("--tag", default=None)
    args = p.parse_args()

    tag = args.tag or (
        f"poc_zw{args.zwin}_ez{args.entry_z}_nd{args.n_days}_h{args.hold_days}_{args.mode}"
    )
    ...
    for sym in args.symbols:
        try:
            ...
            sim = simulate(
                close, premium,
                zwin=args.zwin, entry_z=args.entry_z, n_days=args.n_days,

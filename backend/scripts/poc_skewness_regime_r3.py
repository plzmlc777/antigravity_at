#!/usr/bin/env python3
"""Phase R-3 robustness for skewness_regime (n=200 perm test)."""
...
from scripts.poc_skewness_regime import (  # noqa: E402
    PARADIGM, OUT_DIR, load_ohlcv_5m, simulate_skewness_regime,
)

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("poc_skewness_regime_r3")


def permutation_test(df: pd.DataFrame, n_iter: int, real_alpha: float,
                     **rule_params) -> dict:
    """Shuffle 5m return series — destroys skewness regime structure while
    preserving marginal return distribution."""
    ...
    for it in range(n_iter):
        ...
        sim = simulate_skewness_regime(shuf, train_frac=0.5, **rule_params)
        if sim.get("error"):
...
def main() -> int:
    ...
    p.add_argument("--symbols", nargs="+", default=["UNIUSDT", "LDOUSDT", "AVAXUSDT"])
    p.add_argument("--skew-window", type=int, default=60)
    p.add_argument("--regime-window", type=int, default=8640)
    p.add_argument("--skew-low-pctl", type=float, default=-0.01)
    p.add_argument("--skew-high-pctl", type=float, default=0.95)
    ...
    rule_params = dict(skew_window=args.skew_window, regime_window=args.regime_window,
                       skew_low_pctl=args.skew_low_pctl,
                       skew_high_pctl=args.skew_high_pctl,
    ...
    for sym in args.symbols:
        ...
        real_sim = simulate_skewness_regime(df, train_frac=args.train_frac,
                                             **rule_params)

"""
S51: 5m Williams %R oversold revert + 5m volume z-score filter.

Entry : 5m Williams %R < oversold_thr  AND  5m volume z-score >= volz_min
Sell  : 5m Williams %R > overbought_thr  OR  ema20_break

Hypothesis: Williams %R는 RSI과 다른 momentum oscillator로
range market에서 mean revert 시그널을 더 명확히 잡는 경향 (특히 -80 이하).
거래량 spike 동반 시 false signal 감소.
"""
...
class S51_Williams_Volume_5m(MultiTFBase):
    name = "s51_williams_volume_5m"

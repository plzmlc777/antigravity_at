"""
S46: Triple-TF voting on (VWAP / BB / RSI) — 1m, 5m, 1h.

Entry : sum(buy signals across 3 TF x 3 indicators = 9 votes) >= total_threshold
Sell  : sum(sell signals) >= sell_threshold

Hypothesis: 단일 TF/단일 signal은 noisy. 9개 voting (3 indicator × 3 TF)을 합쳐
임계 이상이면 진입. Diversification of error.
"""
...
class S46_TripleVote_1m5m1h(MultiTFBase):
    name = "s46_triple_vote_1m5m1h"

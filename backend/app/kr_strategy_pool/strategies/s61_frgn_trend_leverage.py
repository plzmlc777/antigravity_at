"""
S61: 122630 (KODEX 레버리지 ETF) 외국인 5일 누적 trend follow.

신호 (D 종가 후 발표):
  frgn_5d_norm = 외국인 5일 누적 순매수 / 직전 20일 평균 거래대금
  consensus_3d = (frgn_3d>0) + (inst_3d>0)   ∈ {0,1,2}

진입: D+1 09:00 시가
  frgn_5d_norm > frgn_thr  AND  consensus_3d >= consensus_min
...
근거: ACF1(1m) ≈ 0 (random walk), 122630 vs 005930 daily corr 0.892,
      외국인 5일 누적 vs fwd_ret_5d corr +0.234 (가장 강한 상관).

검증 (2026-05-02, 1년 데이터 2025-05-02~2026-04-30 / 92,690 1m bars):
  Full 1y          : return +89.34%, sharpe 3.60, win 72.4%, maxDD -8.02%, 60 trades
  IS 6m / OOS 6m   : IS +22.60% (sharpe 2.35) / OOS +54.37% (sharpe 2.80)
  6-fold WF        : 6/6 positive, avg +11.06%/fold (~2m), worst +1.16%, best +26.57%
  Grid 72 combos   : 100% positive (robust plateau, not cliff). DEFAULT rank 7/72.

월 환산 (보수적, WF 기반): 약 +5.5%/월. 단순 1y/12: +7.4%/월. 복리 1y: +5.5%/월.

대조 (post-fix perf_matrix_122630): 기존 1m 풀 (s40-s53) 단일 양수 전략 0개,
oracle ceiling +5.05%/월 → S61 단일 +5.5%/월으로 oracle 돌파.
"""
...
class S61_FrgnTrendLeverage(KrStrategyBase):
    name = "s61_frgn_trend_leverage"
    ...
    PARAMETER_SCHEMA: ClassVar[Dict[str, Any]] = {
        "frgn_thr": {"type": "float", "min": -0.1, "max": 0.5},
        "consensus_min": {"type": "int", "min": 0, "max": 2},
        "hold_days": {"type": "int", "min": 1, "max": 15},
        "sl_pct": {"type": "float", "min": 0.02, "max": 0.20},
        "tp_pct": {"type": "float", "min": 0.05, "max": 0.40},
    }
    ...
    def _build_signals(self, flow: pd.DataFrame):
        ...
        foreign_5d = flow["foreign"].shift(1).rolling(5).sum()
        inst_3d = flow["inst"].shift(1).rolling(3).sum()

        frgn_norm_5d = foreign_5d / avg_trade.replace(0, np.nan)
        frgn_norm_3d = foreign_3d / avg_trade.replace(0, np.nan)
        inst_norm = inst_3d / avg_trade.replace(0, np.nan)
        consensus = (frgn_norm_3d > 0).astype(int) + (inst_norm > 0).astype(int)

        signal_at_d = (
            (frgn_norm_5d > float(self.config["frgn_thr"]))
    ...
    def _trading_days_held(self, current_date: Date) -> int:
        if self._holding_entry_date is None:
            return 0
        return max(0, (current_date - self._holding_entry_date).days)
    ...
    def on_data(self, candle: Dict[str, Any]) -> None:
        ...
        d = candle_dt.date()
        hhmm = candle_dt.strftime("%H:%M") if hasattr(candle_dt, "strftime") else str(ts)[11:16]
        ...
        if hhmm >= str(self.config["exit_minute"]):
            ...
            if held >= int(self.config["hold_days"]):
                qty = self.ctx.holdings.get(self.symbol, 0)
                self.ctx.sell(self.symbol, qty, price=price, metadata={"reason": "time"})
                self._entry_price = None
                self._holding_entry_date = None
                return
            ...
            if inst_now is not None and inst_now < float(self.config["rev_thr"]):
                qty = self.ctx.holdings.get(self.symbol, 0)
                self.ctx.sell(self.symbol, qty, price=price, metadata={"reason": "rev"})
                self._entry_price = None
                self._holding_entry_date = None
                return

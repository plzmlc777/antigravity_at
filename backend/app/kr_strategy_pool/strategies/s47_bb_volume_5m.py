"""
S47: 5m BB lower revert + 5m volume z-score filter (single-TF).

Entry : 5m close < lower BB  AND  5m volume z-score over `volz_window` >= volz_min
Sell  : 5m close > mid BB

Hypothesis: 평균 회귀가 의미 있으려면 그 봉에 비정상적 매수세 (volume spike)가
있어야 한다. 거래량 없는 BB 하단 터치는 weak.
"""
...
class S47_BB_Volume_5m(MultiTFBase):
    name = "s47_bb_volume_5m"

"""숏 수수료 회귀 테스트 + 사본 격차 고정 (정본 엔진 1단계).
...
무엇을 고정하나
  ...
  4) 두 실행기의 숏 수수료 식이 같다 — 2026-08-08 사고("같은 policy, 다른
     실행기, 다른 전략")의 재발 감지
  5) **D2 격차 해소 확인** (3b, 2026-08-13) — 미설정 브래킷을 양쪽 모두
     비활성으로 통일했다. 브래킷은 policy 가 선언한 것만 존재한다.
     반대 방향 위험(선언을 잊으면 손절 없음)은 출하 policy 6경로를 강제해 막는다.
...
class TestKnownDivergence(unittest.TestCase):
    ...
    D2 는 3b(2026-08-13)에서 해소돼 `converges` 로 뒤집혔다 — 그 실패가 곧
    수정의 증거였다. D6 는 정당한 설계 차이라 남는다. 격차가 조용히 사라지거나
    조용히 되살아나지 않게 하는 것이 이 클래스의 목적이다.
    """

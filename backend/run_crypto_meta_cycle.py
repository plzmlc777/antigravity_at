"""
Crypto Meta-Strategy Paper Cycle Runner — Phase 5 wire-in.

Usage:
    cd backend && source venv/bin/activate

    # Create session (1회)
    python3 run_crypto_meta_cycle.py --create --session BTCUSDT_meta_seed \\
        --symbol BTCUSDT --start 2026-05-04 --capital 10000 \\
        --meta-model runs/crypto_paper/models/meta_lgbm_BTCUSDT.pkl

    # Daily cron cycle
    python3 run_crypto_meta_cycle.py --session BTCUSDT_meta_seed
"""
import argparse
import asyncio
import os
...
from app.crypto_strategy_pool.meta_paper_runner import MetaPaperRunner
...
async def main():
    ...
    p.add_argument("--meta-model", default="runs/crypto_paper/models/meta_lgbm_BTCUSDT.pkl")
    p.add_argument("--meta-min-sharpe", type=float, default=0.5)
    p.add_argument("--meta-confidence-min", type=float, default=0.3)
    p.add_argument("--meta-fallback", default="c40_vwap_atr_1m5m")
    ...
    if args.create:
        ...
        print(f"Created crypto meta session: {args.session}")
    ...
    if cycle["mode"] != "cash_hold":
        print(f"  final_equity        ${cycle['final_equity']:,.2f}")
    ...
    if session.last_final_equity is not None:
        print(f"  last_final_equity   ${session.last_final_equity:,.2f}")

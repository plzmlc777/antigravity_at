...
install_service() {
    ...
    if [ -d "$PROJECT_ROOT/venv" ]; then
        echo "  - venv detected..."
        # We DO NOT want to force use venv/bin/uvicorn because it might not be there 
        # (user installed uvicorn successfully in .local, but venv is partial/broken or just for python)
        # So we skip this replacement or check explicitly.
        # But for stability now, let's just stick to what works (system/user uvicorn).
        # We'll comment out the harmful replacements:
...
systemctl enable auto_trading_backend
systemctl enable auto_trading_frontend
systemctl restart auto_trading_backend
systemctl restart auto_trading_frontend
